...
VKOSPI 22-30: 경계 레짐 (high)
  → 방향성 신호 신뢰도 × 0.85 (-15%)
  → 역추세 패턴(reversal) 위양성 증가 주의

VKOSPI > 30:  위기 레짐 (crisis)
  → 모든 방향성 신호 신뢰도 × (0.50 ~ 0.75)
  → 지지/저항선 무력화 (20번 §6 서킷브레이커 참조)
  → 캔들 패턴은 노이즈에 매몰 — 차트 패턴만 제한적 유효

NOTE: 원본 학술 기준 15/25/35에서 KRX 실측 분포 기반 15/22/30으로 보정 (2026-04 calibration).
Doc28 §4 cross-market regime과 일관.
```
...
매수 확인: PCR_5d > 1.3 AND VKOSPI > 22
  → "공포가 가격에 반영됨" 확률 높음
  → 24번 §1의 FearGreed < 0.3 조건과 합산 시 신뢰도 증폭

매도 확인: PCR_5d < 0.5 AND VKOSPI < 15
  → "탐욕이 극에 달함" 확률 높음
  → FearGreed > 0.7 조건과 합산 시 역추세 신호

NOTE: 원본 학술 기준 PCR 1.2/0.6에서 KOSPI200 OI 실측 분포 기반 1.3/0.5로 보정.
```
...
| 58 | VKOSPI_NORMAL_THRESH | 22 | C | GCV | [20, 25] | KRX 분포 보정 (원본 25) |
| 58b | VKOSPI_HIGH_THRESH | 30 | C | GCV | [28, 35] | KRX 분포 보정 (원본 35) |
| 59 | PCR_FEAR_THRESH | 1.3 | C | GCV | [1.0, 1.5] | KOSPI200 OI 보정 (원본 1.2) |
| 60 | PCR_GREED_THRESH | 0.5 | C | GCV | [0.4, 0.8] | KOSPI200 OI 보정 (원본 0.6) |
