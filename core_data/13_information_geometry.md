...
## 8. 전이 엔트로피 — Transfer Entropy

### 8.1 전이 엔트로피 정의 (Definition)

Schreiber (2000)이 제안한 전이 엔트로피는 Granger 인과의 비선형 일반화다.
시계열 X가 시계열 Y의 예측에 제공하는 **방향성 정보량**을 측정한다.
...
여기서:
  y_t^(k) = (y_t, y_{t-1}, ..., y_{t-k+1}): Y의 k-이력(k-history)
  x_t^(l) = (x_t, x_{t-1}, ..., x_{t-l+1}): X의 l-이력

해석: X가 Y 자신의 과거를 넘어 Y의 미래 예측에 제공하는 정보량 (단위: bits 또는 nats)
```

Granger 인과와의 관계:
- Granger 인과: 선형 VAR 모형에서의 예측력 검정
- 전이 엔트로피: 비선형, 분포 무가정(distribution-free)의 일반화
- Barnett, Barrett & Seth (2009): 가우시안 변수에 대해 두 방법은 동치

### 8.2 금융 시장 적용 (Financial Market Application)

**VIX → VKOSPI 정보 전달:**

```
정상 기간 (Normal period):
  TE(VIX → VKOSPI) ≈ 0.02-0.05 nats
  → 미국 변동성 지수가 한국 변동성에 미약한 정보 전달

위기 기간 (Crisis period):
  TE(VIX → VKOSPI) ≈ 0.15-0.30 nats (3-6배 증폭)
  → 글로벌 위기 시 미국→한국 정보 전달이 급격히 강화

역방향:
  TE(VKOSPI → VIX) ≈ 0.01-0.02 nats (비대칭 — 글로벌→로컬 지배적)
```
...
```
대형주 → 소형주 리드:
  TE(반도체 → IT 소형주) > TE(IT 소형주 → 반도체)
  → 대형 반도체가 IT 소형주보다 정보 우위

레짐 전환 신호:
  TE 스파이크 탐지: 60일 이동 평균 대비 2σ 초과 시 → 레짐 전환 신호
  → 정보 흐름 구조의 급변은 시장 구조 변화의 선행 지표
```

### 8.3 엔트로피와 시장 효율성 (Entropy & Market Efficiency)

수익률 분포의 Shannon 엔트로피:

```
H(R) = -Σ p(r_i) × log₂(p(r_i))

완전 효율적 시장 (random): H → 최대 (균등분포에 근사)
예측 가능한 패턴 존재: H < H_max
예측 가능성 지표: Pred = 1 - H / H_max

  Pred = 0: 완전 랜덤 (패턴 없음)
  Pred > 0: 패턴이 존재하여 엔트로피가 최대에 미달
```

signalEngine.js 엔트로피 하한(0.80, [D] 등급)과의 연결:
- 이 파라미터는 최대 엔트로피 대비 이탈 정도를 암묵적으로 측정
- 이론적 근거: 낮은 엔트로피 = 패턴 존재 = 신호의 신뢰도가 높음
- 효율적 시장 가설(EMH)과의 관계: 엔트로피가 최대에 가까울수록 EMH에 부합

### 8.4 조건부 전이 엔트로피 (Conditional Transfer Entropy)

교란 변수 Z를 통제한 조건부 전이 엔트로피:

```
CTE(X → Y | Z) = TE(X → Y) - TE(Z → Y)

적용 예시:
  VIX → KOSPI 정보 전달이 USD/KRW 환율과 독립적인가?

  CTE(VIX → KOSPI | FX) ≈ TE(VIX → KOSPI):
    → VIX 효과가 직접적 (환율 경로를 거치지 않음)

  CTE(VIX → KOSPI | FX) ≈ 0:
    → VIX 효과가 환율을 매개로 전달 (간접적)

해석 주의:
  높은 차원(k, l 증가) 추정은 데이터 요구량이 기하급수적으로 증가
  → 실무에서는 k = l = 1~3이 일반적
  → 커널 밀도 추정 또는 k-NN 엔트로피 추정기 사용
```

### 8.5 정보기하학과의 연결 (Connection to Information Geometry)

전이 엔트로피는 본 문서 §4의 KL 발산으로 표현 가능하다:

```
TE(X → Y) = D_KL( p(y_{t+1} | y_t, x_t) ‖ p(y_{t+1} | y_t) )

기하학적 해석:
  통계적 다양체 S 위에서,
  - p(y_{t+1} | y_t, x_t): X의 정보를 포함한 조건부 분포 (한 점)
  - p(y_{t+1} | y_t):       X의 정보를 제외한 조건부 분포 (다른 점)
  - TE = 이 두 점 사이의 KL "거리"

  TE가 클수록 X의 정보 포함/제외에 따라 다양체 위의 위치가 크게 변함
  → X가 Y의 조건부 분포 형태를 유의미하게 변화시킨다는 의미
```

교차 참조:
- 본 문서 §4: KL 발산과 시장 상태 — TE의 수학적 기반
- 28_cross_market_correlation.md §2.3: Transfer Entropy의 VIX→VKOSPI 적용
- 03_physics.md: 경제물리학에서의 엔트로피 개념
