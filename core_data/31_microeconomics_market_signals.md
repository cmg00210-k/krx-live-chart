...
### 1.1.1 Kyle (1985) 가격 충격 모형: λ 정식 도출

§1.1의 스프레드 분해에서 역선택 비용(s_adverse)은 Kyle (1985)의 λ에 의해 결정된다.
Kyle의 연속경매 모형은 정보 비대칭 하의 가격 형성을 정식화한 시장미시구조 이론의
핵심 결과이다. 아래에서 λ = σ_v / (2σ_u)를 도출한다.

```
모형 설정:
  v ~ N(v̄, σ_v²)     기업의 진정한 가치 (정보거래자만 관찰)
  u ~ N(0, σ_u²)      유동성(잡음) 거래자의 주문
  y = x + u            시장조성자가 관찰하는 총 주문 흐름
  x: 정보거래자의 주문량 (결정 변수)

정보거래자의 최적화 문제:
  max_x  E[(v - p) · x | v]
  s.t.   p = v̄ + λ · y       (시장조성자의 선형 가격결정 규칙)
...
시장조성자의 합리적 기대 조건:
  λ = Cov(v, y) / Var(y)    (OLS 투영 계수)

  Cov(v, y) = Cov(v, x* + u)
            = Cov(v, (v - v̄)/(2λ))    (∵ Cov(v,u) = 0)
            = Var(v) / (2λ)
            = σ_v² / (2λ)
  ...
  양변에 [σ_v²/(4λ²) + σ_u²]를 곱하고 정리:
    λ · σ_v²/(4λ²) + λ · σ_u² = σ_v²/(2λ)
    σ_v²/(4λ) + λ · σ_u² = σ_v²/(2λ)
...
경제학적 해석:
  — σ_v↑ (기업가치 불확실성 증가) → λ↑ (가격 충격 증가)
     ∵ 주문에 정보가 포함될 확률이 높아짐
  — σ_u↑ (유동성 거래 증가) → λ↓ (가격 충격 감소)
     ∵ 정보거래자가 잡음 속에 위장하기 쉬워짐
  — λ는 "정보의 가격": 한 단위의 주문 흐름이 가격을 λ만큼 이동
```

**Kyle λ와 Amihud ILLIQ의 관계:**

λ는 이론적 모수(관찰 불가)이고, Amihud ILLIQ는 실증적 프록시(proxy)이다:

```
이론: λ = σ_v / (2σ_u)           — σ_v, σ_u 미관측
실증: ILLIQ = (1/T) Σ |r_t| / DVOL_t  — OHLCV에서 직접 산출

연결: ILLIQ ∝ λ / P̄   (가격 수준 정규화)
  Kyle λ의 정의에서: Δp = λ · Δy
  수익률로 변환: |r| = |Δp/p| = λ · |Δy| / p
  DVOL = p · V ∝ p · |Δy| (거래대금 ∝ 주문 흐름의 규모)
  ∴ |r| / DVOL ∝ λ / p²

  Hasbrouck (2009): ILLIQ의 cross-section 순위가 Kyle λ 순위와
  Spearman ρ ≈ 0.85-0.92로 높은 상관
```

**KRX 세그먼트별 λ 추정:**

| 세그먼트 | 추정 λ 범위 | ILLIQ 범위 (log) | 해석 |
| ---------- | ------------- | ------------------ | ------ |
| KOSPI 50 대형 | 0.00001-0.0001 | [-7, -5] | σ_u 극대, 가격 충격 최소 |
| KOSPI 200 중형 | 0.0001-0.001 | [-5, -3] | 중간 수준 |
| KOSDAQ 대형 | 0.001-0.005 | [-3, -1] | 정보 비대칭 높음 |
| KOSDAQ 소형 | 0.005-0.01+ | [-1, 1] | σ_u 극소, 극단적 가격 충격 |

**CheeseStock 매핑:** `indicators.js`의 `calcAmihudILLIQ()`는 Kyle λ의 실증적
프록시이다. `appWorker.js`에서 ILLIQ을 log 변환 후 dual-path(LOG_LOW=-3, LOG_HIGH=-1)
로 분류하는 로직은 위 표의 세그먼트 구분과 일치한다.

참고문헌:

- Kyle, A.S. (1985). Continuous Auctions and Insider Trading. *Econometrica*, 53(6), 1315-1335.
- Hasbrouck, J. (2009). Trading Costs and Returns for U.S. Equities: Estimating Effective Costs from Daily Data. *JF*, 64(3), 1445-1477.
...
### 1.4 KRX 가격제한폭(±30%) 하의 사중손실 정량화

가격제한에 도달하면 거래가 계속되지만 균형 가격에 도달하지 못하여 사중손실(DWL)이
발생한다. 이는 가격 상한(ceiling)이 균형가격보다 낮은 표준 미시경제학 모형의
증권시장 적용이다.

```
가격상한(price ceiling)에 의한 사중손실:

  DWL_limit = (1/2) × (P* - P_limit) × (Q_D(P_limit) - Q_S(P_limit))

  P*:       가격제한이 없을 때의 균형가격 (unobserved)
  P_limit:  상한가(P_prev × 1.30) 또는 하한가(P_prev × 0.70)
  Q_D(P_limit): 제한가격에서의 잠재 수요량
  Q_S(P_limit): 제한가격에서의 잠재 공급량

상한가 도달 시 (P_limit < P*):
  — 초과수요 = Q_D(P_limit) - Q_S(P_limit) > 0
  — 매수 호가가 체결되지 못한 물량 → 미실현 거래의 잉여 손실
  — DWL ∝ (P* - P_limit)²  (수요·공급이 선형이면 삼각형 면적)

하한가 도달 시 (P_limit > P*):
  — 초과공급 = Q_S(P_limit) - Q_D(P_limit) > 0
  — 매도 호가가 체결되지 못한 물량
  — DWL ∝ (P_limit - P*)²
```
...
```
P*의 직접 관측은 불가능하지만, 익일 시초가(P_open,t+1)를 프록시로 사용할 수 있다.
Kim & Rhee (1997)에 따르면 KRX에서:

상한가 도달 후:
  P_open,t+1 / P_limit,t - 1 = 평균 -2.3% (KOSPI), -4.7% (KOSDAQ)
  — 과잉반응(overreaction) 시사: P*가 실제로는 P_limit보다 낮을 수 있음
  — 이 경우 가격제한이 DWL을 발생시키면서 동시에 과잉반응을 억제

하한가 도달 후:
  P_open,t+1 / P_limit,t - 1 = 평균 +1.8% (KOSPI), +3.2% (KOSDAQ)
  — 하한가에서도 반전 경향 → 정보 반영 지연

연간 DWL 추정 (KOSDAQ 기준):
  — 상/하한가 도달 빈도: 연 ~2,500건 (KOSDAQ 전체)
  — KOSDAQ은 KOSPI의 약 3배 빈도
  — 건당 평균 미체결 잔량 × (P* - P_limit) / 2 로 추산
  — 연 총 DWL ≈ 수백억 원 수준 (Kim & Rhee 1997 추정 기준)
```

**가격제한의 이중 비효율성:**

1. **정적 DWL**: 제한가 도달 시점의 미체결 잔량에 의한 즉시 손실
2. **동적 비효율**: 정보가 당일 가격에 완전 반영되지 못하고 익일로 이월
   → 가격 발견(price discovery) 지연
3. **자석 효과(magnet effect)**: Du, Liu & Rhee (2009)가 입증한 바와 같이, 가격이
   제한에 접근할수록 투기적 주문이 가속하여 제한 도달 확률을 높임

이러한 비효율에도 불구하고 가격제한은 극단적 변동성을 억제하는 서킷 브레이커 기능을
수행하며, 2015년 ±15% → ±30% 확대는 DWL 감소와 변동성 억제 간의 정책적 트레이드오프
(policy trade-off)를 반영한다.

**CheeseStock 매핑:** `patterns.js`의 priceTarget/stopLoss가 전일 종가 기준 ±30% 범위를
초과하면 당일 도달 불가능하다. 상한가/하한가 도달 빈도가 높은 KOSDAQ 소형주에서는
패턴의 목표가가 2거래일 이상에 걸쳐 달성되는 경우를 고려해야 한다.

참고문헌:

- Kim, K.A. & Rhee, S.G. (1997). Price Limit Performance: Evidence from the Tokyo Stock Exchange. *JF*, 52(2), 885-901.
- Cho, D.D., Russell, J., Tiao, G.C. & Tsay, R. (2003). The Magnet Effect of Price Limits. *JEF*, 11(3), 459-475.
...
### 2.1.1 VPE와 Amihud ILLIQ의 수학적 관계

Marshall의 거래량-가격 탄력성(VPE)과 Amihud (2002) ILLIQ는 역관계에 있다. 이 관계는
수요-공급 탄력성이라는 미시경제학적 개념이 시장미시구조의 유동성 측정으로 직접
연결되는 이론적 다리(theoretical bridge)이다.

```
정의 비교:

  VPE:   ε_VP = (ΔV/V) / (|Δp|/p)     — 가격 1% 변동 시 거래량 % 변화
  ILLIQ: ILLIQ = (1/T) Σ |r_t| / DVOL_t  — 단위 거래대금당 가격 변동

수학적 관계 도출:

  |r_t| ≈ |Δp/p|  (수익률 ≈ 가격 변화율)
  DVOL_t = p_t × V_t  (거래대금 = 가격 × 거래량)

  따라서:
    ILLIQ_t = |Δp/p| / (p × V)

  VPE 정의에서 |Δp/p| = (ΔV/V) / ε_VP 이므로:

    ILLIQ_t ≈ (ΔV/V) / (ε_VP × p × V)

  정상 상태(steady state)에서 ΔV/V ≈ 상수로 근사하면:
...
경제학적 해석:
  — 높은 VPE (탄력적, 대형주) → 낮은 ILLIQ → 유동성 풍부
  — 낮은 VPE (비탄력적, 소형주) → 높은 ILLIQ → 유동성 부족
  — 가격 수준(P)이 높을수록 → 낮은 ILLIQ (거래대금 자체가 큼)
```

**실증 검증 (KRX 횡단면):**

| 세그먼트 | 평균 VPE | 평균 log(ILLIQ) | 이론적 예측 | 부합 |
| --------- | --------- | ---------------- | ----------- | ------ |
| KOSPI 대형 | 3-5 | [-7, -5] | 낮은 VPE → 중간 ILLIQ, 그러나 P 높아 상쇄 | O |
| KOSPI 중형 | 5-8 | [-5, -3] | 높은 VPE × 낮은 P → 중간 ILLIQ | O |
| KOSDAQ 대형 | 8-12 | [-3, -1] | 높은 VPE이나 P 낮아 ILLIQ 상승 | O |
| KOSDAQ 소형 | 15-20+ | [-1, 1] | 매우 높은 VPE이나 P 극히 낮아 ILLIQ 극대 | △ |

KOSDAQ 소형주에서 VPE가 매우 높음에도 ILLIQ가 큰 것은 가격 수준 효과(P 항)가
탄력성 효과(VPE 항)를 압도하기 때문이다. 이는 ILLIQ = k/(P×|VPE|) 공식에서 P가
분모에 직접 들어가는 구조적 결과이며, Amihud 자신도 시가총액 정규화를 권장한
이유이다 (Amihud 2002, p.34).

**CheeseStock 매핑:** `indicators.js`의 `calcAmihudILLIQ()` 함수는 이 연결의 실증적
구현이다. 향후 VPE를 독립적으로 산출하여 ILLIQ과의 교차 검증(cross-validation)에
활용할 수 있다: VPE 대비 ILLIQ가 이론적 예측보다 높은 종목은 일시적 유동성 부족
(liquidity drought) 상태를 시사한다.
