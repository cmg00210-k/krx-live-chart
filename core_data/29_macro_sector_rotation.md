...
**MCS v1 vs MCS v2 조정 (Reconciliation Note):**

위 5요소 MCS는 v1(초기 설계)이다. 실제 구현(`compute_macro_composite.py`)은
8요소 MCS v2를 사용하며, `macro_composite.json`의 `mcsV2` 필드(0-100 스케일)로
저장된다. MCS v2가 authoritative이다.

| MCS v2 요소 | 가중치 | 비고 |
|------------|--------|------|
| CLI (OECD 경기선행지수) | 0.20 | v1에 없음 — 가장 포괄적 선행지표 |
| ESI (경제심리지수) | 0.15 | v1 CSI의 확장 |
| IPI (산업생산지수) | 0.15 | v1에 없음 — 실물경제 대리변수 |
| 소비자신뢰 (BSI proxy) | 0.10 | v1 CSI에서 분리 |
| PMI (BSI 제조업) | 0.10 | v1 w1=0.25에서 축소 |
| 수출 YoY | 0.10 | v1 w3=0.25에서 축소 |
| 실업률 역수 | 0.10 | v1에 없음 — 고용 건전성 |
| 금리스프레드 | 0.10 | v1 w4=0.15에서 축소 |

v1의 EPU(경제정책불확실성)는 v2에서 제거됨 (VIX proxy 사용이 EPU와 중복).
`appWorker.js`는 `_macroComposite.mcsV2`를 소비하며 0-1 vs 0-100 스케일
가드가 적용됨 (line 571-572).
