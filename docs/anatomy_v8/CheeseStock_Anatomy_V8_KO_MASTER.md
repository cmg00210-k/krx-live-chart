...
KRX 가격·수급·파생
  ...
  금융학 (2.6)
    ...
    APT Ridge 회귀 → 5-팩터 노출도 (momentum_60d·beta_60d·value_inv_pbr·log_size·liquidity_20d) — `scripts/mra_apt_extended.py` 오프라인 배치 + `js/aptModel.js` 런타임 클라이언트 (17-col Ridge, lambda=2.0, horizon=5d, n=237,977)
...
| 특성 | 가우시안 ($\alpha$ = ∞) | 멱법칙 ($\alpha$ ≈ 3) |
|------|:-------------------------:|:-----------------------------:|
| ± 3$\sigma$ 빈도 | 0.27% (연 1회) | 1--2% (연 3--5회) |
| ± 5$\sigma$ 빈도 | 6 × 10⁻⁷ | 위기 시 관측됨 |
| ± 10$\sigma$ 빈도 | 10⁻²³ | 1987년 블랙 먼데이 실제 발생 |
...
| 단계 | 공식 | 목적 |
|:----:|------|------|
| 1. Unlever | $\beta$_(U,i) = $\beta$_(L,i) / [1+(1-T_c)(D/E)i] | 각 비교기업 자본구조 제거 |
| 2. Average | $\bar{\beta}$_U = mean($\beta$_(U,i)) | 산업 고유 위험 추정 |
| 3. Re-lever | $\beta$_(L,target) = $\bar{\beta}$_U[1+(1-T_c)(D/E)_(target)] | 분석 대상에 자본구조 재적용 |
...
| $\text{eps\_stability}$ | 이익변동성 감쇠 계수 = 1/(1+$\sigma$_(NI\_growth)/100)  |
...
CheeseStock의 MRA 파이프라인은 APT의 직접 구현이다. **오프라인**(`scripts/mra_apt_extended.py`)은 17열 Ridge 회귀의 설계행렬을 구성한다 — 열 1~12는 패턴 고유 특성(hw, vw, mw, confidence 등), 열 13~17은 APT 팩터(momentum_60d, beta_60d, value_inv_pbr, log_size, liquidity_20d)이며 결과는 `data/backtest/mra_apt_coefficients.json`로 저장된다. **클라이언트**(`js/aptModel.js`)는 V48-SEC Phase 7 P7-001 이후 HMAC+Origin-guard된 `/api/apt` 엔드포인트(`functions/api/apt.js`, `guardGet light_get`)를 통해 이 계수를 fetch한다 — `_signGet` 경로로 서명된 동일-Origin 요청만 허용되어 raw 계수의 공개 접근을 차단하며, `data/backtest/mra_apt_coefficients.json` 정적 URL에는 CDN 오염 제거용 placeholder(`{"moved":"/api/apt","protected":true}`)가 배포된다. **Phase 6 Consumer wiring (P6-002, 2026-04-20)**: `_collectOccurrences`가 `aptModel.predict()`를 호출하여 `occ.aptPrediction`을 orphan field로 저장 (Option C, chain coupling 없음). **Phase 7 P7-001 활성화 (2026-04-21)**: 클라이언트가 5-factor 전체를 실제 값으로 주입하도록 3-pass pipeline으로 재구성 — (a) Pass 1에서 `aptModel.computeFactors(candles, idx, stockMeta)`로 raw 팩터(momentum_60d/beta_60d/value_inv_pbr/log_size/liquidity_20d)를 수집(edge case 가드 강화: `pbr > 0.01`, `count >= 10` 거래일, `idx < 60` per-factor gate, liquidity ×100 unit parity 버그 수정), (b) Pass 2에서 `aptModel.zscoreCohort(rawList)`가 훈련 파이프라인과 동일한 공식(median + MAD×1.4826, winsorize [−3,3])으로 cohort 기반 robust z-score를 1회 계산, (c) Pass 3에서 z-score된 팩터를 predict에 주입하여 `aptPrediction` + `aptFactors` 필드를 occurrence에 저장. `stockMeta = {marketCap, pbr, market}`는 main thread의 `appWorker._getStockMetaForApt()`가 조달하여 postMessage로 Worker에 전달하며(`analysisWorker`가 `backtester._currentStockMeta`에 주입), $\mu$/$\sigma$는 client runtime에만 존재하므로 서버/JSON 추가 IP 노출 zero다. **IC 정밀 측정 6-Layer MVP**: `_computeStats`의 horizon 루프 h∈{1,3,5,10,20} 각각에 `_computeAPTDiagnostic(validOccs, returns, h, candles, reg)`이 실행되어 `stats.aptDiagnostic` 객체를 생성한다 — L1 Pairing(동일 occurrence 집합에서 APT vs 5-col WLS 비교, `nPaired ≥ 20`), L2 Cross-sectional per-date(날짜 정규화 YYYY-MM-DD KST, `nDates ≥ 10`, 미달 시 pooled fallback), L3 ICIR = mean(IC_t)/std(IC_t) annualized × √(252/h), L5 Fisher 95% CI via $z = \tfrac{1}{2}\ln((1+r)/(1-r))$ back-transformed through tanh with `nEff = nDates`, L9 Null contamination split(5/5 non-null full subset vs ≥1 null partial subset). L4 sign consistency는 horizon 루프 종료 후 `_computeAPTMeta(result)`가 h∈{3,5,10} 부호 일치 3/3을 검사하여 `result._aptMeta.signConsistent` + `result._aptMeta.mvpGate`로 집계한다. **MVP GO gate (5/5 충족 필요)**: `icirAnn ≥ 0.3`, `icLift ≥ +0.015` vs 5-col WLS paired, `ci95.lower > 0` @ h=5, `fullFactorRatio ≥ 0.5`, sign consistency h∈{3,5,10} 3/3. 4개 이하 충족 시 HOLD(Phase 8 L6 block bootstrap + L7 factor decomposition 진입), 3개 미만 시 NOGO(APT 재학습). L6/L7/L8/L10은 Phase 8 이월 — L6 Politis-Romano stationary block bootstrap은 within-date clustering 보정, L7 LOO+SFO-IC는 factor별 marginal contribution 진단, L8 27-cell stratification(KOSPI/KOSDAQ × size × vol-regime)은 regime dependency 분리, L10 Harvey-Liu-Zhu (2016) BH-FDR는 5×5 hypothesis family-wise error 보정. Walk-Forward IC = 0.0998(Phase 4-1 offline baseline, 237,977 samples)은 5개 APT 팩터 모두 $p < 0.001$ 유의함을 확인했으며, 유동성($t=-24.69$)이 가장 강력한 가격결정 요인으로 Amihud(2002)의 KRX 적용을 실증한다. Phase 7 client-side 실측값은 배포 후 Chrome DevTools Console의 `window.lastBacktestResults._aptMeta.mvpGate` 관찰로 Phase 7 완료 보고서(`results/anatomy_v8_audit/phase7_completion_report.md`)에 pin된다. **Phase 8 정합성 보정 (2026-04-21)**: Phase C 3-agent 전수감사(code-audit / consistency / self-verify)에서 2 HIGH + 1 P2a 식별 → 즉시 수정. (1) `backtester._analyzeCache`의 캐시 키에 `_currentStockMeta`/`_currentMarket`을 JSON 직렬화하여 포함시켜 동일 candles 배열 참조가 두 종목에 재사용될 때 발생할 수 있는 APT factor 교차 오염을 차단한다. (2) `_aptMeta.mvpGate`에 `betaAvailable` 플래그와 `effectiveFactorCount` 필드를 추가하여, Worker runtime에서 `marketReturns60d` 시장-지수 시계열 plumbing이 부재하는 현재 상태를 진단 자체가 자기 보고하도록 구성한다(Phase 9 backlog: KOSPI/KOSDAQ index-aligned 60일 로그수익률 런타임 주입 → per-occurrence rolling beta 산출). (3) `analysisWorker.js importScripts`가 `aptModel.js`를 의도적으로 제외하는 환경 — Worker 경로에서는 `_computeAPTDiagnostic` 호출을 `typeof aptModel !== 'undefined'` 가드로 감싸 all-zero APT 신호에 대한 무의미한 IC 비교를 차단하고, 진단은 main-thread path에서만 생성하도록 한다. 또한 P7-003 xelatex PDF 빌드는 `scripts/fix_master_greek_for_latex.py`(prose는 `$\mathrm{Greek}$` wrap, fenced code block은 ASCII word로 치환) + `diagram-protect.lua` `\tree*` 매크로 인수 LaTeX-escape 패치(`_`, `$`, `&` 등 10 chars)로 해소되어 sidecar `docs/anatomy_v8/CheeseStock_Anatomy_V8_KO_MASTER.sha256.json`에 md_sha/pdf_sha가 pin되었다 — `verify.py --check anatomy` ALL PASS 전환.
...
KRX 2,645종목 5년(2020--2025) 경험적 발견에서(2026-04-20 pin; `data/backtest/aggregate_stats.json` 245,943 표본), 집계 수준에서 매도 패턴이 매수 패턴을 빈도에서 약 **2.2%p** 상회한다 (bullish 48.9% vs bearish 51.1%, $\Delta$+1,484건). 단, 패턴 군별로는 상반된 방향이 존재하며 `bullishHarami` (+33%), `doubleBottom` (+100%), `morningStar` (+19%)는 오히려 매수 우세이다. 이 완화된 비대칭은 전망이론의 손실회피 및 KRX 구조적 특징(T+2 결제, 가격제한폭, 개인투자자 주도 거래)과 정합하되, 과거 판본의 "10--15%p" 수치는 survivorship 미보정 표본 또는 raw OHLCV bearish-day 비율을 혼동한 추정으로 교정되었다.
...
## 5.2 사용자 전달과 반응형 설계
최종 전달 문제(Last Mile Problem)는 수학적으로 정밀한 이론 출력을 사용자 행동으로 연결하는 과제이다. IC = 0.051, DD = 2.8$\sigma$, MCS v2 = 62.4와 같은 원시 출력은 그 자체로는 사용자에게 직관적이지 않다. CheeseStock의 Stage 5는 이 격차를 티어 시스템, 토스트 알림, 반응형 레이아웃의 세 가지 메커니즘으로 해소한다.
...
| Merton DD = 2.8$\sigma$ | "이 기업이 안전한가?" | 부도거리 범주 표시 |
| WLS $\beta$ = 0.032 | "주가가 오를 것인가?" | 기대수익률 %, 승률 %, 위험/보상 비율 |
...
제2장: 2.2절 시계열분석 — EMA를 지수평활로 정의
       alpha = 2/(n+1), EMA_t = alpha·P_t + (1-alpha)·EMA_{t-1}
...
제3장: DD = (ln(A/D) + (r - sigma²/2)T) / (sigma√T)
...
| $\sigma$_(EWMA)² | EWMA 조건부 분산 ($\lambda$=0.94) | 무차원 (일별²) |
...
| eps_stability | EPS 안정성 계수 1/(1+$\sigma$_NI/100) | appWorker.js:1807 | HHI 부스트 감쇠 (Jensen-Meckling 1976) |
