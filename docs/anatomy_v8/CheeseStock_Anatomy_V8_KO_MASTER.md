...
한국거래소(KRX)는 5개 원천 중 가장 넓은 학문적 분화를 일으키는 데이터 공급원이다. 종가 하나에서 출발한 계산이 7개 학문 분야 — 물리학, 수학, 통계학, 금융학, 경영학, 경제학, 행동재무학 — 로 각각 갈라진다.
...
| 기호 | 의미  |
|:----:|------|
| γt(s) | 시점 t에서 상태 s의 사후 확률 (Baum-Welch E-step)  |
| δt(j) | 시점 t에서 상태 j의 최적 경로 확률 (Viterbi)  |
| k | 자유 매개변수 수 (BIC)  |
| T | 관측 시계열 길이  |
...
이론적 흐름 (9시트): DCF 기업가치 (2.4.1) → MM 무관련성 (2.4.2) → Miller 개인세 (2.4.3) → WACC (2.4.4) → 대리인 비용 (2.4.5) → DuPont 분해 (2.4.5a) → 신호이론 (2.4.6) → EVA (2.4.7) → Kelly 기준 (2.4.8). MM의 완전시장 가정에서 현실적 마찰(세금, 대리인, 정보비대칭)을 하나씩 도입하며, 기업재무 이론의 역사적 발전 경로를 따른다.
...
| 기호 | 의미  |
|:----:|------|
| ROE | 자기자본이익률 = NI / Equity  |
| NPM | 순이익률 = NI / Revenue  |
| AT | 자산회전률 = Revenue / Total Assets  |
| EM | 자본승수 = Total Assets / Equity  |
| NI | 당기순이익 (DART 연결 기준)  |
| NPM_sector | 업종 평균 순이익률 (`sector_fundamentals.json`)  |
| S_(NPM), S_(AT), S_(EM) | 각 구성요소 배점  |
| ROIC | 투하자본수익률 = NOPAT / IC  |
| IC | 투하자본 = 자기자본 + 이자부채  |
...
이 체계는 제3장에서 CONF-F1a(Stovall 섹터 회전), CONF-F7(Taylor Rule Gap), CONF-F9(한미 금리차)의 세 가지 거시 신뢰도 조정 인자와 미시 신뢰도 조정 인자로 구현된다. 각 인자는 거시 및 미시 신뢰도 함수 내에서 패턴별 방향(매수/매도)과 종목의 섹터 분류에 따라 차등 적용되며, 최종 신뢰도는 다층 승수의 곱으로 결정된다(compound floor = 25으로 하한 보장).

## 2.6 금융학적 기초: 자산가격결정에서 신용위험까지[^fin-1]

금융학은 자산의 공정가치를 결정하는 이론적 체계를 제공한다. CAPM 계보는 기대수익률과 위험 프리미엄을 정의하여 기술적 분석의 기준선을 설정하고, 채권·옵션·신용위험 모형은 교차시장 신호를 생성하여 패턴 신뢰도를 다차원적으로 조정한다. 본 절은 효율적 시장에서 다요인 가격결정, 교차시장 신호까지 자산가격결정의 전체 계보를 15개 시트와 요약으로 추적한다.
...
축소 추정량은 원시 승률을 범주 전체 평균($\mu_{\text{grand}}$) 방향으로 당기며, 표본 크기가 작을수록 축소 강도가 커진다. $N_0 = 35$는 KRX 2,700종목 × 46 패턴의 경험적 분산 구조에서 최적화된 축소 강도로, 패턴을 캔들(평균 ~43%)과 차트(평균 ~45%) 두 범주로 분리하여 범주 내 축소를 수행한다.

$$\theta_{\text{post}} = \frac{n \cdot \theta_{\text{raw}} + N_0 \cdot \mu_{\text{grand}}}{n + N_0}$$

표본이 충분히 클 때($n \gg N_0$) 사후 추정치는 원시 승률에 수렴하고, 표본이 극소일 때($n \ll N_0$) 범주 평균에 수렴한다. 이 메커니즘은 Stein(1956)의 축소 추정량이 3차원 이상에서 MLE를 지배한다는 결과의 직접적 응용이며, 2,700종목 × 46 패턴이라는 고차원 추정 문제에서 이론적으로 보장된 개선이다.
...
제2장의 이론은 그 자체로는 추상적이다. 볼츠만 분포와 GARCH 모형이 실제로 2,700개 종목의 차트 위에서 작동하려면, 이론을 구체적인 지표·패턴·신호로 변환하는 과정이 필요하다. 본 장은 CheeseStock에 구현된 32개 지표, 46종 패턴(캔들 35 + 차트 11), 31개 복합 신호 각각이 어떤 학술 이론에서 출발하여 어떤 경로로 코드에 도달했는지를 추적한다.

| 절 | 주제 | 핵심 내용 |
|:--:|------|----------|
| 3.1 | Indicators (지표) | 32개 기술적 지표의 학술 출처와 수학적 정식화 |
...
32개 지표는 학술적 출처, 수학적 정식화, 구현 세부, 하류 소비자와 함께 문서화된다. 각 지표 카드는 학문적 계보, 핵심 공식, 기호 주석, CheeseStock 적용을 체계적으로 기술한다.
...
**캔들스틱 패턴 (Candlestick Patterns).** 35종의 캔들스틱 패턴(단봉 11 + 쌍봉 12 + 삼봉 8 + 다중봉 4)은 1~5봉 단위의 단기 수급 심리를 포착한다.

<!-- newpage -->

### 3.2.1 단봉 패턴 (Single-Bar Patterns)

Nison(1991, *Japanese Candlestick Charting Techniques*)은 혼마 무네히사(18세기 오사카 도지마 미곡거래소)의 캔들 분석 체계를 서양 시장에 도입하였다. 단봉 패턴은 한 봉의 실체(body)와 그림자(shadow) 비율로 해당 시점의 매수-매도 심리 균형을 포착한다.

도지(doji)는 시가와 종가가 거의 같아 실체가 극소화된 봉으로, 매수와 매도 세력의 균형 상태를 나타낸다. 잠자리형 도지와 비석형 도지는 긴 한쪽 그림자로 방향성 거부를 시사한다. 해머(hammer)와 유성(shooting star)은 긴 그림자가 한 방향의 가격 탐색 후 거부(rejection)를 시사하며, Morris(2006, *Candlestick Charting Explained*)가 확인 규칙을 정교화하였다. 마루보즈(marubozu)는 그림자 없이 한 방향으로의 강한 확신을 보여주고, 팽이형(spinning top)과 긴다리 도지는 높은 불확실성을 반영한다.

모든 임계값은 ATR 정규화된다(Wilder 1978).
...
> 이전 Stage 데이터: $\textcolor{stageOneMarker}{O_t, H_t, L_t, C_t}$는 Stage 1 데이터 계층(KRX pykrx)에서 수집된 OHLCV 시계열이다.

> 참조: 제2장 2.7절 (행동재무학 --- 군집행동, 처분효과), 제2장 2.1절 (통계역학).
...
KRX 2,704종목 5년(2020--2025) 경험적 발견에서, 매도 패턴이 매수 패턴을 승률에서 10--15%p 지속적으로 상회한다. 이 매도 편향은 전망이론의 손실회피 및 KRX 구조적 특징(T+2 결제, 가격제한폭, 개인투자자 주도 거래)과 부합한다.
...
| 절 | 주제 | 핵심 내용 |
|:--:|------|----------|
| 3.3.1 | 개별 신호 분류 | 20개 감지 함수 → 31개 이산 신호 |
...
CheeseStock의 신호 엔진은 20개 감지 함수로부터 31개 개별 신호를 생성한다. 16개 기술적 지표 감지 함수(MACD, RSI, BB, 거래량, OBV, 일목, 허스트, StochRSI, Stochastic, 칼만, CCI, ADX, Williams %R, CUSUM, 이진 세분화, ERP)와 4개 파생상품/교차자산 감지 함수(선물 베이시스, PCR, 투자자 수급, ERP)가 매수/매도/중립의 이산 신호를 출력한다.
...
| 학술 개념 | 구현 함수/상수 | 적용 영역 |
|-----------|--------------|----------|
| 다중 출처 확인 (Murphy 1999) | `js/signalEngine.js` `COMPOSITE_SIGNAL_DEFS` | 31개 복합 신호 정의 |
| 무게 증거 원칙 (Pring 2002) | `js/signalEngine.js` `_matchCompositeSignals()` | 필수+선택 조건 결합 |
...
V23은 KRX 2018--2024 월별 데이터에서 추정한 4×4 요인 상관행렬 $\Sigma_F$ (VOL--CREDIT $= 0.72$, VOL--FX $= 0.65$, CREDIT--FX $= 0.58$, FLOW는 역부호)를 상수로 고정하고, Kish (1965)의 유효표본크기 공식을 변환하여 동시발동 요인의 실효 자유도 $N_{\text{eff}}$를 산출한다.
...
예: VOL + CREDIT + FX 3개 요인이 동시 발동하여 $\Sigma_F$의 고유값이 $(2.3,\, 0.9,\, 0.8)$일 때 $N_{\text{eff}} \approx 1.3$, 하한 floor $= 0.892$ (기존 독립 가정 하의 0.795 대비 $+12$\%p 완화)에 해당한다. PCA 예산 함수가 Worker result, fallback, drag의 3개 신뢰도 체인에 모두 적용되며, 이로써 기존 9-Layer 신뢰도 체인(V22-B 이전)은 10-Layer로 확장된다 (감지 수학 3.4절 참조).
...
클램프 범위의 설정은 실증적 근거에 기반한다. 거시 승수의 상한 1.30은
거시 요인이 패턴 신뢰도를 최대 30%까지 상향할 수 있음을 의미하며,
하한 0.70은 극단적 거시 악화 시에도 30% 이상의 감산을 허용하지 않는다.
이 대칭 범위 [0.70, 1.30]은 상·하방 동일 30% 한도로 설계되었으며,
미시 승수의 비대칭 [0.55, 1.15]과 대비된다. 미시 클램프의 비대칭은
행동재무학의 손실 회피(Kahneman and Tversky 1979)와 일관되며,
유동성 충격(Amihud ILLIQ)이 상방보다 하방에 더 큰 영향을 미치는
시장 미시구조의 특성을 반영한다.
...
신뢰도 체인 10개 계층 --- 데이터 파이프라인 참조표

아래 표는 각 계층이 어떤 데이터 원천으로부터 수신하여 승법적 조정을
적용하는지를 추적한다. 실행 순서는 정보 계층 원칙(information hierarchy
principle)에 따라 시장 전체 맥락에서 종목 고유 요인 방향으로 진행된다.
이는 제1장 데이터 원천에서 제3장 신뢰도 산출까지의 전체 데이터 흐름을
완결한다.

| 계층 | 학문 기반 | 데이터 출처 | 조정 범위 |
|:----:|----------|-----------|:-----:|
| 0 시장맥락 | 행동재무학 | market_context.json (CCSI, 외국인 순매수, 어닝시즌) | 개별 조정 |
| 1--2 RORO | 국제금융 | VKOSPI/VIX, 신용, USD/KRW, MCS, 수급 | [0.92, 1.08] |
| 3 거시 | 경제학 | 거시 경제 지표, 채권 수익률, 통계청 데이터 (7팩터) | [0.70, 1.30] |
| 4 미시 | 미시경제학 | OHLCV 비유동성(런타임), HHI 집중도 상수 | [0.55, 1.15] |
| 5 파생 | 금융공학 | 파생상품 요약, 투자자 수급, 옵션 분석, ETF, 공매도 | [0.70, 1.30] |
| 6 Merton | 신용위험 | 재무제표 + OHLCV 변동성 (Bharath-Shumway 간편법) | [0.75, 1.15] |
| 7 Phase8 | 통계학/거시 | MCS v2, HMM 레짐, 외국인 모멘텀, IV/HV, straddle | MCS/HMM 기반 |
| 8 생존편향 | 통계학 | 매수 패턴 Winsorized 할인 | [0.92, 1.0] |
| 9 PCA예산 | 다변량통계 | Kish N_eff + Longin-Solnik 비대칭 극단상관 | 동적 예산 |
| Floor | --- | 복합 하한 (Tukey 1977 Winsorized product) | 하한 25 |
...
_confBefore = confidence                     // PCA 예산용 초기값 보존
// Layer 0: 시장맥락 (CCSI, 외국인, 어닝시즌)
confidence = applyMarketContext(confidence)
// Layer 1-2: RORO 3-체제 분류 + 방향 편향
confidence *= clamp(roroMult,   0.92, 1.08)
markFactorsAfterRORO()                       // 10-key Set 등록
// Layer 3: 거시 7팩터
confidence *= clamp(macroMult,  0.70, 1.30)
markFactorsAfterMacro()                      // 10-key Set 등록
// Layer 4: 미시 (ILLIQ, HHI)
confidence *= clamp(microMult,  0.55, 1.15)
// Layer 5: 파생상품 6요인
confidence *= clamp(derivMult,  0.70, 1.30)
// Layer 6: Merton DD
confidence *= clamp(mertonMult, 0.75, 1.15)
// Layer 7: Phase 8 (MCS v2, HMM, 외국인, IV/HV, straddle)
confidence = applyPhase8(confidence)
// Layer 8: 생존자 편향 조정
confidence = _applySurvivorshipAdjustment(patterns)  // [0.92, 1.0]
// Layer 9: PCA 예산 한도
confidence = applyPCABudget(confidence, _confBefore)
// Floor: 복합 하한
confidence = max(confidence, 25)
confidence = clamp(confidence, 10, 100)      // 최종 범위 제약
```

RORO가 거시(Layer 3)보다 앞에 실행되는 이유는 정보 계층 원칙에 따른다.
RORO는 시장 전체의 위험선호(risk appetite) 체제를 분류하여 후속 거시 팩터의
해석 맥락을 설정한다. V22-B의 적용 요인 10-key Set 가드는 RORO에서
소비된 요인(변동성, 신용)이 Layer 3에서 이중 적용되는 것을 차단한다.

| 학술 개념 | 구현 함수/상수 | 적용 영역 |
|-----------|---------------|-----------|
| 시장 맥락 조정 (Layer 0) | `_applyMarketContextToPatterns()` | CCSI, 외국인 순매수, 어닝시즌 3요인 |
| RORO 3-체제 (Layer 1--2) | `_classifyRORORegime()` + `_applyRORORegimeToPatterns()` | 위험선호 체제 분류 + 방향 편향, 클램프 [0.92, 1.08] |
| 거시 신뢰도 조정 (F1~F9) | `_applyMacroConfidenceToPatterns()` | 7개 거시 요인 승법적 적용, 클램프 [0.70, 1.30] |
| 미시 신뢰도 조정 (M1~M3) | `_applyMicroConfidenceToPatterns()` | 3개 미시 요인 승법적 적용, 클램프 [0.55, 1.15] |
| Stovall 섹터 매핑 | `_STOVALL_CYCLE` (appState.js) | KSIC 대분류 -> 경기순환 4단계 민감도 |
...
기존 승법적 신뢰도 체인은 변동성 국면과 무관하게 정적 cap [10, 95]를 12곳에서 하드코딩하여 사용하였다. V22-B부터 ATR 국면 분류 함수가 ATR(14)의 252-day 분위수 p25/p75로 시장을 고/중/저 3국면으로 분류하고, 동적 cap 조회 함수가 각 신뢰도 축(confidence, macro, pred)마다 국면별 cap을 반환한다. 고변동성 국면에서는 범위가 [25, 75]로 수축되어 극단 신뢰도의 드로다운 증폭을 억제하고, 저변동성 국면에서는 [5, 95]로 확장되어 미세 시그널의 수용폭을 넓힌다. 이 비직관적 방향(고변동성에서 cap 축소)은 KRX 2018--2024 실증 드로다운 경로에서 도출된 것으로, 기존 정적 cap이 단일 국면 가정 하에서 과적합되어 있었음을 시사한다.

아울러 V22-B는 적용 요인 10-key Set 가드를 도입하여 동일 요인(변동성·신용·테일러갭·MCS·HMM·환율·유동성·외국인수급·옵션·부도거리)이 거시 승수 체인과 RORO 승수 체인에서 이중 적용되는 구조적 double-counting을 차단한다. 각 요인 적용 직후 Set에 등록되며, 후속 체인이 Set hit 시 해당 요인을 건너뛴다. V23은 이 구조 위에 Kish--Longin-Solnik PCA 효과량 예산(3.4.6절)을 얹어 잔여 독립성 위반을 추가 교정하며, 이로써 신뢰도 체인은 9-Layer에서 10-Layer로 확장된다. 상세 수식과 백테스트 결과는 기술적 분석 도출 요약(3.7.1절)의 V22--V25 개정 하위절에 기술되어 있다.
...
Layer 0~6의 승법적 조정이 완료된 후, 신뢰도 체인의 후반부에서
거시경제 국면(regime)의 복합 판단이 적용된다. Layer 7(Phase 8 결합),
Layer 8(생존자 편향), Layer 9(PCA 예산), 복합 하한(Floor)으로 구성된다.

시장 맥락 조정(Layer 0)은 신뢰도 체인의 첫 번째 계층으로, Baker and
Wurgler(2006)의 투자자 심리 지표(CCSI), Richards(2005)의 외국인 자금
흐름, 어닝시즌 불확실성의 3개 요인을 반영한다. CCSI가 극단적으로 낙관적
(>115)이면 매수 패턴을 할인하고 매도 패턴을 부스트하며, 극단적으로
비관적(<85)이면 반대 방향으로 조정한다. 외국인 순매수 추세는 5일 이동평균
기준으로 방향을 판단하며, 어닝시즌(1/4/7/10월)에는 실적 불확실성을
반영하여 전체 패턴의 신뢰도를 소폭 할인한다.

Phase 8 결합(Layer 7)은 Hamilton(1989)의 은닉 마르코프 모형(HMM)으로 추정된
...
투자자 수급 데이터에 Baum-Welch EM 알고리즘을 적용하여 2-state 레짐(bull/bear, 미분류 시 중립 폴백)을
추정하고, 레짐별 신뢰도 승수를 통해 매수/매도 패턴에 차등 승수를
적용한다.
...
R2 신용스프레드의 AA- 가중치는 V22-B에서 0.10으로 복원되었다.
기존 RX-06에서 0.05로 축소하였으나, V22-B의 적용 요인 10-key Set 가드가
CONF-계층1 F3(신용 국면)과의 이중 반영을 명시적으로 차단하므로
가중치 완화가 불필요해졌다.
...
| 3.6.4 | V22-V25 방법론 개정 | 동적 cap, CUSUM 수정, PCA 예산 |
| 3.6.5 | Contrarian 승격 8패턴 | OOS 승률 기반 BH-FDR 검정 통과 |
...
2,700종목 동시 스캔 환경에서는 패턴 내 다중비교(33+ 패턴 $\times$ 5 수평)에 더하여 종목 간 다중비교 문제가 발생한다. Harvey, Liu, and Zhu(2016)의 교차자산 보정을 적용하여 $q_{\text{cross}} = q / \sqrt{N_{\text{stocks}}} \approx 0.05 / \sqrt{2700} \approx 0.00096$으로 조정한다. 이 엄격한 임계값은 `backtester.js`의 `_applyBHFDR()` 후단에서 적용된다.
...
| ILLIQ 슬리피지 | `backtester.js` `_getAdaptiveSlippage()` | 종목별 Amihud ILLIQ 기반 슬리피지 조정 |
...
(1) OOS 시간 분할 (V22-B) --- 기존 랜덤 hold-out 방식은 KRX 상승장 편향을 제거하지 못하여 과적합 위험이 상존했다. V22-B부터 OOS 승률 산출 스크립트가 2025-11-01 기준 시간 분할(Lo 2002)을 강제한다. Train 구간 2025-04-11 \~ 2025-10-31 ($N_{\text{train}} = 41{,}778$), test 구간 2025-11-03 \~ 2026-04-03 ($N_{\text{test}} = 41{,}232$)로 train:test $= 50.3 : 49.7$ 대칭이다. OOS 데이터에는 패턴별 OOS 승률, 축소 승률, 95% 신뢰구간, $p$-값이 기록되며, 런타임은 OOS $\to$ LIVE $\to$ SHRUNK 우선순위로 승률을 주입한다.

(2) 변동성 국면 동적 적응 (V22-B) --- 하드코딩 cap [10, 95] 12곳이 동적 cap 조회 함수로 교체되었다. ATR 국면 분류 함수가 ATR(14)의 252-day 분위수 p25/p75로 시장을 고/중/저 국면으로 분류하고, 고변동성 국면에서는 cap이 [25, 75]로 수축되어 극단 신뢰도 드로다운 증폭을 억제한다. 저변동성 국면에서는 [5, 95]로 확장되어 미세 시그널 수용폭을 넓힌다. 이 비직관적 방향은 KRX 2018--2024 실증 드로다운 경로에서 도출된 것으로, 기존 정적 cap이 단일 국면 가정 하에서 과적합되어 있었음을 시사한다.

(3) PCA 효과량 예산 (V23) --- PCA 효과량 예산(3.4.6절)에서 서술된 Longin--Solnik (2001) 비대칭 극단상관과 Kish (1965) 유효표본크기 프레임워크가 신뢰도 체인에 반영되었다. PCA 예산 함수가 Worker result / fallback / drag의 3개 체인에 모두 적용되며, downside floor $= \exp(-0.10\sqrt{N_{\text{eff}}})$, upside ceiling $= \exp(+0.12\sqrt{N_{\text{eff}}})$의 비대칭 예산으로 동시발동 요인의 복합 cap을 교정한다. V22-B 이전의 9-Layer 체인은 이 단계 추가로 10-Layer가 된다.

(4) Contrarian 승격 --- $\textit{confidencePred} = 100 - \textit{dirWr}$ (V25) --- V22-B 초기 OOS 분석에서 8개 bullish 패턴이 일관되게 $\textit{dirWr} < 50$을 기록하였고, V23은 이를 예측 신뢰도 null로 배제하여 방향 정확도 메트릭 오염을 방지하였다. V25는 1-sided binomial 검정과 Benjamini--Hochberg FDR ($q = 0.10$)으로 이 8개 패턴의 반대방향 예측력을 재검정하였고, 전원 통과(Bonferroni $\alpha = 0.05$에서도 통과)에 따라 contrarian 플래그를 부여하였다. 런타임에서 해당 패턴의 예측 신뢰도는 $\textit{confidencePred} = 100 - \textit{dirWr}$로 반전되며, 동일 품질 스케일링·cap·페널티 체인을 그대로 상속한다. 이론적 기반은 Lo (2004) AMH의 crowding 포화 경로와 Jegadeesh (1990) 1개월 단기반전이다 (효율적 시장가설과 적응적 시장가설, 2.6.1절).

### 3.6.5 8개 Contrarian 승격 패턴 (출처: OOS 승률 데이터, 2026-04-10)
...
유의사항: 강세감싸기(bullishEngulfing)의 Cohen's $g \approx 0.04$는 trivial effect에 해당하여 실무적 유의성이 제한적이다. 모닝스타(morningStar)는 marginally powered ($n_{\text{test}} = 277$)이며 차기 OOS 사이클에서 재평가 대상이다. 3개 차트 패턴(이중바닥, 역헤드앤숄더, 상승삼각형)은 IS 표본이 $n_{\text{IS}} \leq 4$로 극소하여 IS-OOS 델타 검정이 무의미하고, OOS 유의성에만 근거한다.

(5) V25-B CUSUM 변화점 3-수정 + 전수 재실행 --- V24에서 CUSUM 변화점 감지의 TypeError 수정[^cusum-v24]으로 비로소 활성화된 cusumBreak 시그널은 V25-B에서 3개의 추가 정합성 수정을 받았다: ① 수익률 공간 인덱스 $\to$ 캔들 공간 교정(+1 오프셋), ② 변동성 국면 파라미터 passthrough로 $h = 3.5$ 적응형 임계값 활성화, ③ 방향 전파(neutral $\to$ 변화점 방향 기반 buy/sell). 2,646 종목 전수 재실행(245,103 패턴 표본)에서 방향 정확도 = 0.4928로 baseline 대비 거의 변화가 없었는데, 이는 batch 런타임의 시그널 방향이 contrarian 반전(런타임 전용)의 영향을 받지 않기 때문이며, contrarian 승격의 실질 효과는 차기 OOS 사이클에서 측정된다.
...
| 절 | 주제 | 핵심 내용 |
|:--:|------|----------|
| 3.7.1 | 기술적 분석 도출 요약 | 이론→지표→패턴→신호의 전체 도출 경로 |

### 3.7.1 기술적 분석 도출 요약 (Technical Analysis Derivation Summary)
제3장은 제2장의 7개 학문 분야에서 도출된 이론을 32개 지표, 46종 패턴(캔들 35 + 차트 11), 31개 개별 신호, 31개 복합 신호, 6개 계층 신뢰도 체인, 그리고 다중 통계 검증 체계로 변환하는 과정을 문서화하였다. 이 변환 과정에서 5개 학문 분야가 추가적으로 관여한다: 기술적 분석(Dow-Hamilton-Rhea, Nison, Edwards-Magee), 계량경제학(WLS, Ridge, HC3), 베이지안 통계학(베타-이항 사후 축소), 금융공학(다요인 신뢰도 체인), 실험설계(Walk-Forward, BH-FDR).
...
|:----:|-----------|-----------------|--------|
| 3.1 Indicators | 32개 지표 (3.1.1~3.1.32절) | 통계학, 물리학, 금융학, 수학 | 가격 파생 수치 |
| 3.2 Patterns | 46종 패턴 (캔들 35 + 차트 11) + S/R | 행동재무학, 기술적 분석 | 패턴 감지 + 품질 점수 |
...
| 3.5 신뢰도 체인 | CONF-계층1~4 (거시/미시/국면/파생) | 경제학, 금융학, 금융공학 | macroMult~mertonMult |
...
| 46종 패턴 감지 | `js/patterns.js` `patternEngine.analyze()` | OHLCV→패턴 식별 |
...
> 이전 Stage 데이터: $\textcolor{stageThreeMarker}{\text{patterns}}$, $\textcolor{stageThreeMarker}{\text{confidence}}$, $\textcolor{stageThreeMarker}{\text{priceTarget}}$, $\textcolor{stageThreeMarker}{\text{stopLoss}}$는 Stage 3 (제3장 3.2–3.5절)에서 산출된 패턴 감지 결과 및 6개 계층 신뢰도 조정 점수이다. 본 Stage(제4장)는 이 값들을 소비하여 시각 부호로 변환하며, 수정하지 않는다.
...
임의의 종목 $s$와 시점 $t$에 대해, 분석 출력은 데이터 전달 경로(WS 소켓 또는 HTTP fetch)에 무의존적이다. 이는 Stage 3의 모든 지표(32개), 패턴(46종), 신호(31개 복합), 신뢰도 조정(CONF-계층0..9)이 OHLCV 배열과 JSON 매크로 파일만을 입력으로 취하고 전달 메커니즘을 참조하지 않기 때문에 구성적으로 보장된다.
...
$$\text{Toast} = f_{\text{compress}}\!\left(\bigcup_{i=1}^{46} \text{Pattern}_i \times \prod_{k=0}^{9} \text{CONF}_k\right) \to \text{"N개 패턴 감지됨"}$$
