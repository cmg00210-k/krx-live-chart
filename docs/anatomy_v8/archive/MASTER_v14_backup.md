...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| α | 꼬리지수(tail exponent) | 무차원 | 본 Stage |
| x_(min) | 멱법칙 하한 임계값 | KRW 또는 % | 본 Stage |
| α̂ | 힐 추정량으로 측정된 꼬리지수 | 무차원 | **Stage 1** 가격 데이터 |
...
| 학술 개념 | 구현 함수/상수 | 적용 영역 |
|-----------|---------------|-----------|
| α̂ < 4 → `isHeavyTail` | `patternEngine` Hill 추정 | EVT 보정 볼린저 밴드 활성화 |
...
| 학술 개념 | 핵심 수식 | 적용 영역 |
|-----------|-----------|-----------|
| 볼츠만 분포 / 시장 온도 | P(E) ∝ e^(-E/k_BT) | 변동성 국면 분류 |
| 이징 모형 / 군집행동 | H = -JΣ si sj - hΣ si | 행동 패턴 필터링 |
| 멱법칙 꼬리 (α ≈ 3) | P(x) ∼ x^(-α) | 꼬리 두께 측정 |
| 자기조직 임계성 | P(S) ∼ S^(-τ) | 차트 패턴 돌파 크기 |
| 프랙탈 스케일링 / 자기유사성 | X(ct) =^(d) c^H X(t) | 추세 지속성 측정 |
...
| 패턴i | Stage 1에서 감지된 캔들/차트 패턴 | 범주 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| Xn | 시점 n의 가격 (또는 로그가격) | KRW | 본 Stage |
| Φt | 시점 t까지의 정보 집합 | — | 본 Stage |
| μ | 상수 드리프트 (EMH 하) | %/일 | 본 Stage |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| St | 시점 t의 주가 | KRW | 본 Stage |
| μ | 드리프트 (기대수익률) | 연율 % | 본 Stage |
| σ | 확산 계수 (변동성) | 연율 무차원 | 본 Stage |
| Wt | 표준 위너 과정 | √{시간} | 본 Stage |
| Pt | Stage 1에서 수집된 실시간/일봉 가격 | KRW | **Stage 1** |
...
| R(n) | 윈도우 n에서 누적 편차의 범위 | KRW | 본 Stage |
| S(n) | 윈도우 n의 모집단 표준편차 (÷ n) | KRW | 본 Stage |
| D = 2 - H | 프랙탈 차원 | 무차원 | 본 Stage |
| Pt | Stage 1 가격 시계열 | KRW | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| β̂ | 회귀 계수 벡터 | 다양 | 본 Stage |
| X | 설계행렬 (피처) | 다양 | 본 Stage |
| W | 가중행렬 diag(w1,...,wn) | 무차원 | 본 Stage |
| λ | 릿지 정규화 매개변수 (GCV 선택) | 무차원 | 본 Stage |
| I | 단위행렬 | — | 본 Stage |
| y | 종속변수 (패턴 수익률) | % | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| x̂t | 상태 추정치 (평활 가격) | KRW | 본 Stage |
| Kt | 칼만 이득 (Kalman gain) | 무차원 | 본 Stage |
| Pt | 추정 오차 공분산 | KRW² | 본 Stage |
| Q | 과정 잡음 | KRW² | 본 Stage |
| R | 관측 잡음 | KRW² | 본 Stage |
| zt | 관측 가격 | KRW | **Stage 1** |
...
| 학술 개념 | 핵심 수식 | 적용 영역 |
|-----------|-----------|-----------|
| 확률론 (콜모고로프 공리) | (Ω, F, P) | 기초적 프레임워크 |
| 베이즈 정리 | P(A|B) ∝ P(B|A)P(A) | 다중 패턴 신뢰도 융합 |
| 마르팅게일 이론 | E[Xn₊1|Xn] = Xn | 패턴 분석의 정당성 |
| 랜덤워크 / GBM | dS = μ S dt + σ S dW | 가격 시뮬레이션 |
| 이토 해석학 | 이토 보조정리 | 변동성 추정, BSM PDE |
| 프랙탈 기하학 / 허스트 | E[R(n)/S(n)] = Cn^H | 추세 지속성, 전략 선택 |
| 릿지 회귀 | (X^TWX + λ I)⁻¹X^TWy | 패턴 수익 예측 |
| 칼만 필터 (LQG) | x̂t = x̂t₋1 + Kt(zt - x̂t₋1) | 적응형 가격 평활화 |
...
| σt² | 조건부 분산 | %² | 본 Stage |
| ω | 장기 분산 수준 | %² | 본 Stage |
| α | ARCH 계수 (충격 반응) | 무차원 | 본 Stage |
| β | GARCH 계수 (분산 지속성) | 무차원 | 본 Stage |
| λ = 0.94 | EWMA 감쇠 (RiskMetrics 관례) | 무차원 | 본 Stage |
| rt | 로그수익률 ln(Pt/Pt₋1) | 무차원 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| ξ | 형상 매개변수 (꼬리 유형) | 무차원 | 본 Stage |
| α̂ | 힐 꼬리지수 추정량 | 무차원 | 본 Stage |
| k | 상위 순서통계량 수 | 정수 | 본 Stage |
| u | POT 임계값 | % | 본 Stage |
| X₍i₎ | 정렬된 절대수익률 순서통계량 | % | **Stage 1** |
...
| 학술 개념 | 구현 함수/상수 | 적용 영역 |
|-----------|---------------|-----------|
| α̂ < 4 | `isHeavyTail` flag | EVT 보정 볼린저 활성화 |
...
| λ_(decay) = 0.995 | 반감기 ≈ 139 거래일 | 무차원 | 본 Stage |
| hii | 지렛점 (모자행렬 대각) | 무차원 | 본 Stage |
| ei | OLS 잔차 | % | 본 Stage |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| RVt^((d)) = rt² | 일별 실현 분산 | %² | 본 Stage |
| RVt^((w)) = 1/5Σi₌0⁴ rt₋i² | 주간 성분 | %² | 본 Stage |
| RVt^((m)) = 1/MΣi₌0^(M-1) rt₋i² | 월간 성분 (M=21 KRX) | %² | 본 Stage |
| rt | 일별 로그수익률 | 무차원 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| θ̂ | 매개변수 추정치 | 다양 | 본 Stage |
| f(x;θ) | 확률밀도함수 | 무차원 | 본 Stage |
| xi | 관측 데이터 | 다양 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| St | 은닉 상태 (시장 국면) | 범주 | 본 Stage |
| aij | 전이확률 (i → j) | 무차원 | 본 Stage |
| bj(Ot) | 방출확률 | 무차원 | 본 Stage |
| Ot | 관측 수익률 시계열 | % | **Stage 1** |
...
| 학술 개념 | 핵심 수식 | 적용 영역 |
|-----------|-----------|-----------|
| GARCH / EWMA | σt² = λσt₋1² + (1-λ)rt₋1² | 조건부 변동성 |
| 힐 꼬리지수 | α̂ = k / Σ[ln X₍i₎ - ln X₍k₊1₎] | 두꺼운 꼬리 감지 |
| GPD 꼬리 적합 | Pickands-Balkema-de Haan POT | EVT 기반 VaR |
| WLS + 릿지 회귀 | (X^TWX + λ I)⁻¹X^TWy | 패턴 수익 예측 |
| HC3 강건 표준오차 | (1-hii)² 보정 | 이분산성 보정 |
| 틸-센 강건 추정 | β̂ = median(yj-yi)/(xj-xi) | 이상치 저항 추세선 |
| HMM (바움-웰치) | Hamilton (1989) 2상태 마르코프 | 시장 국면 분류 |
| HAR-RV | RVt₊1^((d)) = β0 + β_d RVt^((d)) + β_w RVt^((w)) + βm RVt^((m)) | 다중척도 변동성 예측 |
...
| Δ NWC | 순운전자본 변동 | 원 | 본 Stage |
| EBIT | DART 영업이익 | 원 | **Stage 1** |
| EPS | 주당순이익 | 원/주 | **Stage 1** |
| β_(KRX) | KRX 베타 (CAPM 산출용) | 무차원 | **Stage 1** |
...
| R_D | 타인자본비용 | % | 본 Stage |
| 자본총계 | DART 자기자본 장부가 | 원 | **Stage 1** |
| 이자부채 | DART 단·장기차입금 합계 | 원 | **Stage 1** |
...
| Ts | 자기자본 소득 유효 개인세율 | % | 본 Stage (Case B: ≈ 0) |
| D | 부채 시장가치 | 원 | 본 Stage |
| 자본총계 | DART 자기자본 | 원 | **Stage 1** |
| 이자부채 | DART 차입금·사채 합계 | 원 | **Stage 1** |
...
| T_c | 법인세율 | % | 본 Stage |
| β_L | 레버리지 베타 | 무차원 | 본 Stage |
| β_U | 무레버리지 베타 | 무차원 | 본 Stage |
| R_f | 국고채 3Y/10Y 수익률 | % | **Stage 1** (ECOS API) |
| β | KRX CAPM 베타 | 무차원 | **Stage 1** (`compute_capm_beta.py`) |
| 실효세율 | DART 법인세비용/세전이익 | % | **Stage 1** |
...
| β* | 최적 인센티브 강도 | 무차원 | 본 Stage |
| ρ | 대리인 위험회피 계수 | 무차원 | 본 Stage |
| σ² | 산출물 분산 (환경 불확실성) | -- | 본 Stage |
| Δ f | 노력에 의한 산출 차이 | -- | 본 Stage |
| ROE | 자기자본이익률 (financials.json) | % | **Stage 1** |
| NI | 당기순이익 | 억원 | **Stage 1** |
| RPRR | 관계사 매출 비중 (tunneling proxy) | 무차원 | 본 Stage |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| θ | 기업 품질 유형 (private information) | -- | 본 Stage |
| C(θ) | 시그널 비용 함수 | KRW | 본 Stage |
| γ0, γ1 | 경영자 고정보상, 가치연동 계수 | KRW, 무차원 | 본 Stage |
| L | 파산 시 경영자 패널티 | KRW | 본 Stage |
| Dt | t기 배당 | KRW/주 | 본 Stage |
| τ | 목표 배당성향 (target payout ratio) | 무차원 | 본 Stage |
| c | 배당 조정 속도 (speed of adjustment) | 무차원 | 본 Stage |
| DPR | 배당성향 (배당금/순이익) | % | **Stage 1** |
| EPS growth | 주당순이익 성장률 (DART) | % | **Stage 1** |
...
| NOPAT | DART 영업이익 × (1-T) | 원 | **Stage 1** |
| IC | DART 자본총계 + 이자부채 | 원 | **Stage 1** |
| WACC | CAPM 기반 산출 WACC | % | **Stage 1** (`compute_capm_beta.py`) |
...
| μ | 기대 초과수익률 | % | 본 Stage |
| σ² | 수익률 분산 | %² | 본 Stage |
| Σ | 수익률 공분산 행렬 | — | 본 Stage |
| p_(backtest) | 패턴별 역사적 승률 | 확률 | **Stage 1** (`backtester.js`) |
| b_(backtest) | 패턴별 역사적 손익비 | 무차원 | **Stage 1** (`backtester.js`) |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| f(·) | 패턴 품질 점수 함수 | 0~1 | 본 Stage |
| g(·) | 재무 조정 함수 (EVA, PBR, WACC) | 0~1 | 본 Stage |
| h(·) | 매크로 환경 조정 함수 | 0~1 | **2.5절** |
| f*_(Kelly) | 패턴별 Kelly 비율 | 0~1 | **Stage 1** |
| f*max | Tier별 최대 포지션 한도 | 0~1 | 본 Stage |
...
| 절 | 학술 개념 | 핵심 수식 | 적용 영역 |
|----|-----------|-----------|-----------|
| 2.4.1 | DCF (Damodaran 1995) | V = Σ FCFt/(1+WACC)^t + TV | `updateFinancials()` PER/PBR 역산 |
| 2.4.2 | MM 정리 (1958/1963) | V_L = V_U + T_c D | D/E → WACC 계산, IC 분모 |
| 2.4.3 | Miller 개인세 (1977) | G_L = [1-(1-T_c)(1-Ts)/(1-T_d)]D | 소액주주 비과세 Case B WACC 보정 |
| 2.4.4 | WACC + Hamada (1972) | β_L = β_U[1+(1-T_c)D/E] | `compute_eva.py`, `compute_capm_beta.py` |
...
| 2.4.7 | EVA (Stewart 1991) | EVA = NOPAT - WACC × IC | `compute_eva.py`, `financials.js` EVA 행 |
...
| π | 현재 CPI 인플레이션율 | %YoY | 통계청 |
| π* | 인플레이션 목표 | %p | BOK 공식(2.0%) |
| a_π | 인플레 반응 계수 | 무차원 | Taylor(1993): 0.50 |
| a_y | 산출량 갭 반응 계수 | 무차원 | Taylor(1993): 0.50 |
| π_(CPI) | CPI 전년동월비 | %YoY | **Stage 1** |
| Taylor\_gap | 테일러 갭 | %p | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| κ | 자본이동성 | 십억원/%p | Lane & Milesi-Ferretti(2007) |
| r* | 해외이자율(Fed Funds Rate) | %p | FRED |
| E[Δ e] | 기대환율변동률 | %YoY | 시장 컨센서스 |
| i_(Fed) | 미국 연방기금금리 | %p | **Stage 1** |
| e_(USD/KRW) | 원/달러 환율 | 원/달러 | **Stage 1** |
...
| α | 가격경직성 파라미터 | 무차원 | Blanchard & Fischer(1989): 1.20 |
| Yn | 자연산출량(잠재GDP) | 십억원 | BOK 추정 |
...
| CLI | OECD 경기선행지수 | 지수(100=추세) | **Stage 1** |
| PMI | 제조업 구매관리자지수 | 지수(50=확장/수축) | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| wj | 제j 구성요소 가중치 | 무차원 | 설계값, [C][L:GCV] |
| zj | 정규화된 구성요소 값 | [0,1] | 각 API 소스 |
| CLI | OECD 경기선행지수 | 지수 | OECD Stats |
| ESI | 경제심리지수 | 지수 | KOSIS |
| IPI | 산업생산지수 | 지수 | 통계청 |
| MCSv2 | MCS v2 복합점수 | 0-100 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| θ | Calvo 가격경직도 | 무차원 | Kim & Park(2016): 0.75 |
| β | 할인인자 | 무차원 | 표준: 0.99 |
| κ | NKPC 기울기 | 무차원 | Gali & Gertler(1999): 0.05 |
| σ | 기간간 대체탄력성의 역수 | 무차원 | 표준 RBC 파라미터 |
| φ | 노동공급 탄력성의 역수 | 무차원 | 표준 RBC 파라미터 |
| π_(CPI) | CPI 전년동월비 | %YoY | **Stage 1** |
...
| k_T | 조세 승수 | 무차원 | 도출값 |
| s | 한계 누출률 (1-c1(1-t)+m) | 무차원 | 도출값: 1.0375 |
| k_(BB) | 균형재정 승수 | 무차원 | Haavelmo(1945) 정리 |
| ZLB | 제로금리 하한 | %p | r ≤ 0.75\% (한국) |
...
| ē | 장기 균형환율 | 원/달러 | PPP 추정 |
| F / S | 선물환율 / 현물환율 | 원/달러 | 외환시장 |
| θ | 환율 수렴 속도 | 1/기간 | Dornbusch(1976) |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| β1 | Level (장기 수준) | %p | NSS 추정: 2.5-4.5%(한국) |
| β2 | Slope (기울기) | %p | β2 > 0: 역전 |
| β3 | Curvature (곡률) | %p | 중기 경기 기대 |
| λ | 감쇠 매개변수 | 년 | NSS 모형 파라미터 |
| y_(10Y-y_{3Y)} | 국고채 10Y-3Y 스프레드 | %p | **Stage 1** |
| YC_phase | 수익률곡선 국면 | 범주형 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| St | 시점 t의 은닉 레짐 | 범주형 | Hamilton(1989) |
| μs, σs | 레짐 s의 평균/표준편차 | % | Baum-Welch 추정 |
| aij | 레짐 i → j 전이확률 | 무차원 | 전이행렬 |
| γt(s) | 사후 레짐 확률 | [0,1] | EM E-step |
| hmmRegimeLabel | HMM 레짐 라벨 | bull/bear/sideways | **Stage 1** |
...
| 학술 개념 | 구현 함수/상수 | 적용 영역 |
|-----------|---------------|-----------|
| 3-state 레짐 승수 | `REGIME_CONFIDENCE_MULT` (appState.js L394) | bull: buy×1.06/sell×0.92, bear: 역방향 |
...
| ε_(VP) | 거래량-가격 탄력성 (VPE) | 무차원 | 본 Stage |
| OHLCV | 일봉 시가-고가-저가-종가-거래량 | KRW, 주 | **Stage 1** |
| ADV | 평균 일간 거래대금 (Average Daily Value) | 억원 | **Stage 1** |
| s_(adverse) | 역선택 비용 (Glosten-Milgrom 1985) | % | 본 Stage |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| si | 기업 i의 시장점유율 | 무차원 | 본 Stage |
| NE | 등가기업수 (Numbers Equivalent) | 개 | 본 Stage |
| L | Lerner 독점력 지수 | 무차원 | 본 Stage |
| ε_d | 시장 수요의 가격탄력성 | 무차원 | 본 Stage |
| marketCap | 시가총액 (ALL\_STOCKS) | 억원 | **Stage 1** |
| eps_stability | 이익변동성 감쇠 계수 = 1/(1+σ_(NI\_growth)/100) | 무차원 | 본 Stage |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| α | 정보 이벤트 발생 확률 | 무차원 | 본 Stage |
| μ | 정보 거래자 도착률 | 건/일 | 본 Stage |
| ε_b, εs | 비정보 매수/매도 도착률 | 건/일 | 본 Stage |
| c_(info) | 정보 수집 비용 | KRW | 본 Stage |
| ρ | 위험회피 계수 (risk aversion) | 무차원 | 본 Stage |
| ai | 자산 i에 배분된 주의 용량 | bits/기간 | 본 Stage |
| ADV | 평균 일간 거래대금 (주의 프록시) | 억원 | **Stage 1** |
| 외국인 보유비중 | 글로벌 분석가 커버리지 프록시 | % | **Stage 1** |
...
|------|-----------|---------------|---------------|-----------|
| 2.5.1 IS-LM | Hicks(1937) IS-LM 균형 | Hicks(1937), Hansen(1953) | 비교정학 → 이벤트별 conf 차등 | `_applyMacroConfidence` |
| 2.5.2 Taylor Rule | Taylor(1993) 준칙 + gap | Taylor(1993), Rudebusch(2002) | Taylor gap → CONF-F7 | 구현 완료 |
| 2.5.3 Mundell-Fleming | IS-LM-BP 개방경제 | Mundell(1963), Fleming(1962) | 한미 금리차 → CONF-F9 | 구현 완료 |
| 2.5.4 AD-AS | 총수요-총공급 4충격 | Blanchard(2017) | 레짐별 패턴 유형 차등 | `conf_adj` 테이블 |
| 2.5.5 Stovall | 섹터 회전 4국면 | Stovall(1996) | 국면-섹터 승수 → CONF-F1a | `_STOVALL_CYCLE` |
| 2.5.6 MCS v2 | 8요소 복합점수 | 설계값 + OECD/KOSIS | MCS → Phase 8 conf | `_applyPhase8Confidence` |
| 2.5.7 NKPC | Calvo 가격경직성 | Calvo(1983), Gali-Gertler(1999) | θ → 통화정책 전달 크기 | 이론적 기반 |
| 2.5.8 재정승수 | 케인즈 승수 + 구축효과 | Blanchard-Perotti(2002), Barro(1974) | k_G < 1 → 재정 conf 제한 | `conf_fiscal=1.03` |
...
| π_(CPI) (cpi_yoy) | 2.16% | 2.5.2, 2.5.7 | `taylorDetail` |
| Taylor_gap | -0.65\%p | 2.5.2, 2.5.10 | CONF-F7 |
| i_(Fed) (fed_rate) | 3.64% | 2.5.3 | CONF-F9 |
...
|------|------|------|------|
| Φt | 시점 t까지의 정보 집합 | — | 본 Stage |
| εt₊1 | 마르팅게일 차분 (예측불가 충격) | KRW | 본 Stage |
| ACF(k) | k-차 자기상관함수 | 무차원 | 본 Stage |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| wi | 종목 i의 포트폴리오 가중치 | 무차원 | 본 Stage |
| σij | 종목 i,j 간 공분산 | \%² | 본 Stage |
| R* | 목표 기대수익률 | % | 본 Stage |
| σ_(downside) | 하방 편차 (MAR 기준) | % | 본 Stage |
| R_f | 무위험이자율 (KTB 3Y) | %/yr | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| βi | 종목 i의 시장 베타 | 무차원 | 본 Stage |
| αi | Jensen의 알파 (위험조정 초과수익) | %/일 | 본 Stage |
| E[Rm] | 시장 포트폴리오 기대수익률 | % | 본 Stage |
| σm | 시장 수익률 표준편차 | % | 본 Stage |
| R²i | 결정계수 (체계적 위험 비율) | 무차원 | 본 Stage |
| R_f | 무위험이자율 (KTB 10Y) | %/yr | **Stage 1** |
| βi | CAPM 베타 (사전 계산) | 무차원 | **Stage 1** |
...
| α_(i,ZB) | Zero-Beta 보정 알파 | % | 본 Stage |
| βi | 시장 베타 (capm_beta.json) | 무차원 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| β_(i,M) | 시장 베타 (표준 CAPM과 동일) | 무차원 | 본 Stage |
| βi,k | 상태변수 k에 대한 헤지 베타 | 무차원 | 본 Stage |
| λk | 상태변수 k의 위험 프리미엄 | %/yr | 본 Stage |
| Δ sk | 상태변수 k의 혁신(innovation) | 변수별 상이 | 본 Stage |
| J(W,s,t) | Merton 간접 효용 함수 | — | 본 Stage |
| BOK rate | 한국은행 기준금리 (상태변수 s1) | % | **Stage 1** |
| VKOSPI | 변동성 지수 (상태변수 s2) | % | **Stage 1** |
| USD/KRW | 원/달러 환율 (글로벌 위험선호) | 원 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| Mt₊1 | 확률적 할인 인자 (SDF) | 무차원 | 본 Stage |
| δ | 시간 할인 인자 (0 < δ < 1) | 무차원 | 본 Stage |
| γ | 상대적 위험회피계수 (CRRA) | 무차원 | 본 Stage |
| Δ c | 로그 소비 성장률 ln(Ct₊1/Ct) | 무차원 | 본 Stage |
| ψ | 시점간 대체탄력성 (EIS) | 무차원 | 본 Stage |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| bik | 종목 i의 팩터 k 로딩 | 무차원 | 본 Stage |
| Fk | 팩터 k의 서프라이즈 (innovation) | 변수별 상이 | 본 Stage |
| λk | 팩터 k의 위험 프리미엄 | %/yr | 본 Stage |
| εi | 고유 충격 (idiosyncratic shock) | % | 본 Stage |
| K | 공통 팩터 수 (K ≪ N) | 정수 | 본 Stage |
| liquidity_(20d) | 20일 거래 회전율 | 무차원 | **Stage 1** |
| momentum_(60d) | 60일 수익률 | % | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| MKT\_RF | 시장 초과수익률 (Rm - R_f) | % | 본 Stage |
...
| B/M ratio | 자본총계 / 시가총액 | 무차원 | **Stage 1** |
| marketCap | 시가총액 (index.json) | 억원 | **Stage 1** |
...
| D_(Mac) | Macaulay 듀레이션 | 년 | 본 Stage |
| D_(mod) | 수정 듀레이션 | 년 | 본 Stage |
| KTB 3Y/10Y | 국고채 3년/10년 금리 | %/yr | **Stage 1** |
...
| T | 잔존만기 | 년 | 본 Stage |
| r | 무위험이자율 | %/yr | 본 Stage |
| σ | 변동성 (연율화) | %/yr | 본 Stage |
| N(·) | 표준정규분포 CDF | 무차원 | 본 Stage |
| σ_(IV) | 내재변동성 (VKOSPI) | %/yr | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| Δ | 기초자산 가격 민감도 | 무차원 | 본 Stage |
| Γ | Delta의 변화율 (볼록성) | 1/원 | 본 Stage |
| Θ | 시간가치 감쇠 | 원/일 | 본 Stage |
| ν | 변동성 민감도 (Vega) | 원/% | 본 Stage |
| N'(·) | 표준정규분포 PDF | 무차원 | 본 Stage |
| σ_(IV) | VKOSPI 내재변동성 | %/yr | **Stage 1** |
| σ_(RV) | 역사적 실현변동성 | %/yr | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| λ | Kyle 가격 충격 계수 | 원/주 | 본 Stage |
| σv | 정보 가치 변동성 | 원 | 본 Stage |
| σu | 잡음 거래량 표준편차 | 주 | 본 Stage |
| ILLIQt | Amihud 비유동성 측도 | 1/원 | 본 Stage |
| DVOLt | 거래대금 (가격 × 거래량) | 원 | 본 Stage |
| μ | 정보거래자 비율 | 무차원 | 본 Stage |
| rt, volumet | 일별 수익률, 거래량 | %, 주 | **Stage 1** |
...
| σ_V | 자산가치 변동성 | %/yr | 본 Stage |
| E | 자기자본 시장가치 (시가총액) | 원 | 본 Stage |
| DD | 부도거리 (표준편차 수) | 무차원 | 본 Stage |
| PD | 부도확률 | % | 본 Stage |
| 시가총액 | index.json marketCap | 억원 | **Stage 1** |
| 부채총계 | financials/{code}.json | 원 | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| λ(t) | 위험 강도 (hazard rate) | 1/yr | 본 Stage |
| Q(T) | 만기 T까지 생존 확률 | 무차원 | 본 Stage |
| R | 부도 시 회수율 (recovery rate) | 무차원 | 본 Stage |
| P_(risky) | 위험 채권 가격 | 원 | 본 Stage |
...
| 2.6.2 | MPT | SR = (E[Rp]-R_f)/σp | `backtester.js` 위험 통계 | 완전 구현 |
| 2.6.3 | CAPM | E[Ri] = R_f + βi(E[Rm]-R_f) | `calcCAPMBeta()` | 완전 구현 |
| 2.6.4 | Zero-Beta | E[Ri] = E[R_z]+βi(E[Rm]-E[R_z]) | `_SHORT_BAN_PERIODS` | 부분 구현 |
| 2.6.5 | ICAPM | E[Ri]-R_f = β_Mλ_M+Σβkλk | `_applyPhase8Confidence` | 완전 구현 |
| 2.6.6 | CCAPM | 1 = E[Mt₊1(1+Ri)] | 이론 참조만 | 미구현 |
| 2.6.7 | APT | E[Ri] = R_f + Σ bikλk | `mra_apt_extended.py` | 완전 구현 |
| 2.6.8 | FF3/5 | Ri-R_f = α+βMKT+sSMB+hHML | `build_ff3_factors()` (Python) | 완전 구현 |
| 2.6.9 | Bond Pricing | P = Σ C/(1+y)^t + F/(1+y)^n | `compute_bond_metrics.py` | 완전 구현 |
| 2.6.10 | BSM | C = SN(d1)-Ke^(-rT)N(d2) | `compute_options_analytics.py` | 완전 구현 |
| 2.6.11 | Greeks/VRP | VRP = σ_(IV)² - σ_(RV)² | `calcVRP()` | 완전 구현 |
| 2.6.12 | Microstructure | ILLIQ = |r|/DVOL | `calcAmihudILLIQ()` | 부분 구현 |
| 2.6.13 | Merton DD | DD = [ln(V/D)+(r-0.5σ²)T]/(σ√{T}) | `_calcNaiveDD()` | 완전 구현 |
| 2.6.14 | Reduced-Form | λ(t) hazard rate | 구현 없음 | 미구현 |
...
| λ = 2.25 | 손실회피 계수 (K&T 1979) | 무차원 | 본 Stage |
| δ = 0.25 | KRX 보호 계수 (가격제한폭+T+2) | 무차원 | 본 Stage |
| SL_(base) | 기본 손절매 수준 | KRW | **Stage 1** |
| TP_(base) | 기본 목표가 수준 | KRW | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| CSADt | 횡단면 절대 편차 | % | 본 Stage |
| Ri,t | 종목 i의 수익률 | % | 본 Stage |
| Rm,t | 시장 수익률 | % | 본 Stage |
| γ2 | 군집 계수 (음이면 군집) | 무차원 | 본 Stage |
| Ri,t, Rm,t | Stage 1 수익률 데이터 | % | **Stage 1** |
...
| 기호 | 의미 | 단위 | 출처 |
|------|------|------|------|
| θ_(raw) | 원시 승률 | 무차원 | 본 Stage |
| N0 = 35 | 축소 강도 (경험적 베이즈 최적) | 건수 | 본 Stage |
| μ_(grand) | 범주별 총평균 (캔들 ~43%, 차트 ~45%) | 무차원 | 본 Stage |
...
| 손실회피 λ | K&T (1979), λ=2.25 | 손절매 확대 |
| 앵커링 편향 | Tversky-Kahneman (1974) | 지지/저항 수준 |
| CSAD 군집 | Chang et al. (2000) γ2 < 0 | 극단 군중 → 역발상 |
| 베타-이항 사후 | θpost = (nθ_(raw)+N0μ)/(n+N0) | 승률 축소 |
...
본 장은 제2장의 이론을 코드로 변환하는 과정을 다루며, 다음 학문 분야가 관여한다.

| 학문 분야 | 적용 대상 | 핵심 원칙 |
|----------|----------|----------|
| **기술적 분석** (Technical Analysis) | 지표 31종, 패턴 32종 | Dow-Hamilton-Rhea (1900~1932), Nison (1991), Edwards-Magee (1948) |
| **계량경제학** (Econometrics) | WLS 회귀, IC 측정, 릿지 정규화 | Reschenhofer (2021), Grinold-Kahn (2000), Hoerl-Kennard (1970) |
| **베이지안 통계학** (Bayesian Statistics) | 사후 승률 축소, EB 방법 | Efron-Morris (1975), 베타-이항 사전분포 |
| **금융공학** (Financial Engineering) | 신뢰도 체인 7계층, 위험관리 | APT 다요인 프레임워크 → 6계층 신뢰도 조정 |
| **실험설계** (Experimental Design) | Walk-Forward, BH-FDR 보정 | Pardo (2008), Benjamini-Hochberg (1995) |

이 학문 분야들이 결합하여, 원시 OHLCV 데이터를 **학술적으로 검증 가능한 매매 신호**로 변환한다. 각 지표·패턴·신호는 아래의 이론 출처 카드(lineage card) 형식으로 문서화된다.

---

## 3.1 지표 이론 출처 카드

지표 모듈에 구현된 각 지표는 학술적 출처, 수학적 정식화, 구현 세부,
그리고 후속 분석 모듈(downstream consumer)와 함께 문서화된다.

---

### I-01: 단순이동평균 (SMA)

**이론적 기원:** 수학 -> 기술통계학 -> 산술평균
...
**활용처:** 신호 S-1 (이동평균 교차), S-2 (이동평균 정렬), 스토캐스틱 %D 평활,
CCI 평균편차, 복합 신호.
...
**활용처:** MACD (I-19), EWMA 변동성 (I-26), 변동성 국면 장기 EMA.
...
> **공식 구성항목 Data 참조 — 볼린저 밴드 (I-03)**
>
> | 변수 | 정의 | 단위 | 데이터 출처 |
> |------|------|------|-----------|
> | closes | 종가 배열 | 원 (KRW) | pykrx OHLCV |
> | $n$ | SMA 기간 (20) | 봉 수 | 상수 [A] Bollinger (2001) |
> | $k$ | 시그마 배수 (2.0) | 무차원 | 상수 [A] Bollinger (2001) |
> | $\sigma_{\text{pop}}$ | 모집단 표준편차 | 원 (KRW) | 런타임 산출 ($\div n$, 베셀 X) |
...
**이론적 기원:** 기술적 분석 -> 모멘텀 오실레이터 -> Wilder
...
> **공식 구성항목 Data 참조 — RSI (I-04)**
>
> | 변수 | 정의 | 단위 | 데이터 출처 |
> |------|------|------|-----------|
> | $P_t$ | 종가 배열 | 원 (KRW) | pykrx OHLCV |
> | $n$ | 기간 (14) | 봉 수 | 상수 [A] Wilder (1978) |
> | $AvgGain$, $AvgLoss$ | 와일더 평활 | 원 (KRW) | 재귀 산출 |
> | 70/30 | 과매수/과매도 경계 | 무차원 (0-100) | 상수 [A] Wilder (1978) |

**활용처:** 신호 S-5 (RSI 영역), S-6 (RSI 괴리), StochRSI (I-21),
...
> **공식 구성항목 Data 참조 — ATR (I-05)**
>
> | 변수 | 정의 | 단위 | 데이터 원천 / 경로 |
> |------|------|------|-------------------|
> | $H_t$ | 당일 고가 | 원 (KRW) | pykrx OHLCV 고가 |
> | $L_t$ | 당일 저가 | 원 (KRW) | pykrx OHLCV 저가 |
> | $C_{t-1}$ | 전일 종가 | 원 (KRW) | pykrx OHLCV 종가 |
> | $n$ | 기간 (14) | 봉 수 | 상수 [A] Wilder (1978) |
> | 폴백 | 0.02 (ATR 불가) | 무차원 | KRX 중앙값, 상수 [C] |

**활용처:** 모든 패턴 감지, 모든 손절/목표 산출, 지지/저항 클러스터링 허용오차,
신뢰도 조정, OLS 추세 정규화.
...
**이론적 기원:** 기술적 분석 -> 거래량 분석 -> Granville
...
지표 중 하나이다 (Barber-Odean 2008 관심 이론, 제2장 2.7절).

**활용처:** 신호 S-20 (OBV 괴리), 복합 신호 buy_volRegimeOBVAccumulation.
...
**이론적 기원:** 물리학/프랙탈 -> 장기 의존성 -> Mandelbrot
...
**활용처:** 신호 S-11 (허스트 국면: H > 0.6 추세, H < 0.4 평균회귀).
...
**활용처:** EVT 기반 손절매 최적화 (백테스터).

---

### I-12: CAPM 베타

**이론적 기원:** 금융학 -> 자산가격결정 -> 자본자산가격결정모형
...
**활용처:** 백테스터 B-6 (젠센 알파), 재무 패널 표시, 베타 로드 함수.
...
**활용처:** VRP (I-14), 변동성 국면 분류.

---

### I-14: VRP (분산 위험 프리미엄)

**이론적 기원:** 금융학/파생상품 -> 변동성 -> 위험 프리미엄
...
**활용처:** 신뢰도 요인 F8, RORO 요인 R1 (VKOSPI 경유).
...
**활용처:** 백테스터 WLS 회귀 예측, OLS 추세 (I-17).

---

### I-15a: HC3 강건 표준오차

**이론적 기원:** 통계학 -> 이분산-일치 추정
...
**활용처:** 백테스터 WLS 계수의 t-통계량.

---

### I-16: GCV 람다 선택

**이론적 기원:** 통계학 -> 모형 선택 -> 일반화 교차검증
...
> **공식 구성항목 Data 참조 — MACD (I-19)**
>
> | 변수 | 정의 | 단위 | 데이터 출처 |
> |------|------|------|-----------|
> | closes | 종가 배열 | 원 (KRW) | pykrx OHLCV |
> | fast=12, slow=26 | EMA 기간 | 봉 수 | 상수 [A] Appel (1979) |
> | sig=9 | 시그널 EMA 기간 | 봉 수 | 상수 [A] Appel (1979) |
...
**활용처:** 신호 S-3 (MACD 교차), S-4 (MACD 괴리), 복합 신호.

---

### I-20: 스토캐스틱 오실레이터

**이론적 기원:** 기술적 분석 -> 모멘텀 -> Lane
...
**활용처:** 신호 S-10, 복합 buy_wrStochOversold.

---

### I-21: 스토캐스틱 RSI

**이론적 기원:** 기술적 분석 -> 복합 오실레이터 -> Chande-Kroll
...
**이론적 기원:** 기술적 분석 -> 편차 기반 오실레이터 -> Lambert
...
**활용처:** 신호 S-13, 복합 buy_cciRsiDoubleOversold.

---

### I-23: ADX / +DI / -DI

**이론적 기원:** 기술적 분석 -> 추세 강도 -> Wilder
...
**활용처:** 신호 S-14, 복합 buy_adxGoldenTrend, sell_adxDeadTrend.

---

### I-24: Williams %R

**이론적 기원:** 기술적 분석 -> 모멘텀 오실레이터 -> Williams
...
**이론적 기원:** 금융학/위험 -> 조건부 변동성 -> RiskMetrics
...
**활용처:** 변동성 국면 분류 (I-27), RORO 복합.

---

### I-27: 변동성 국면 분류

**이론적 기원:** 금융학/국면 -> 변동성 비율 분류
**수식:**
...
**이론적 기원:** 통계학 -> 품질관리 -> 순차분석
**핵심 논문:** Page (1954); Roberts (1966).

변동성 적응형 임계값으로 확장: 고변동성 -> h=3.5, 저변동성 -> h=1.5.

**활용처:** 신호 S-17, 복합 buy_cusumKalmanTurn.

---

### I-30: 이진 세분화

**이론적 기원:** 통계학 -> 구조변화 감지 -> BIC 기반
...
패턴 모듈에 구현된 21개 이상의 캔들스틱 패턴은 일본 쌀 거래 전통에서
...
- CHART_TARGET_ATR_CAP = 6 [B] --- EVT 99.5% VaR 경계
...
폴백: close x 0.02 (KRX 대형주 중앙값 일별 ATR/종가 비율).
...
| S-22 | PCR 역발상 | PCR > 1.2 (공포 → 매수), < 0.6 (탐욕 → 매도) | 62 | Pan-Poteshman (2006) |
| S-23 | 수급 정렬 | 매수/매도 정렬 + 외국인 순매수 ±5000억 | 58-68 | Choe-Kho-Stulz (2005) |
| S-24 | ETF 심리 | 강세 심리 + 레버리지 비율 > 3.0 → 역발상 매도 | 55 | Cheng-Madhavan (2009) |
| S-25 | 공매도 비율 | 시장 공매도 비율 > 8% → 숏커버 랠리 (매수) | 56-63 | Desai et al. (2002) |
| S-26 | ERP | ERP = (1/PER) x 100 - KTB10Y; > 5.5% → 매수 | 60 | Fed 모형, Asness (2003) |
| S-27 | VKOSPI 국면 | > 30 위기(0.60x), 22-30 고위(0.80x) | 할인 배수 | Whaley (2009) |
| S-28 | HMM 레짐 | 강세/약세/횡보 + 외국인 모멘텀 | Phase8 적용 | Hamilton (1989) |
| S-29 | CUSUM 변화점 | 최근 20봉 내 변화점 감지 | 52 | Page (1954) |
| S-30 | 이진 세분화 | 역추세 신호 할인 (0.85x) | 할인 배수 | Bai-Perron (1998) |
| S-31 | HAR-RV | **지표만 구현, 신호 미구현** | N/A | Corsi (2009) |

> **데이터 흐름 주의사항:**
> - S-29, S-30, S-31은 외부 데이터 파일을 사용하지 않고 OHLCV 캔들에서 직접 산출된다.
> - S-21의 베이시스 데이터는 파생상품 요약과 베이시스 분석 두 데이터 원천에서 병합된다.
> - 표본 데이터인 경우 투자자 수급 및 공매도 데이터를 분석에서 제외한다. 오류 상태인 경우 옵션 분석 데이터를 폐기한다.
...
각 계층의 클램프가 순차적으로 적용되므로, 최악의 경우 기본 신뢰도 65가 65 × 0.70 × 0.55 × 0.70 × 0.75 × 0.92 = 12.4로 하한에 근접하며, 최선의 경우 65 × 1.25 × 1.15 × 1.30 × 1.15 × 1.08 = 100 상한에 도달한다.
...
릿지 $\lambda$ = GCV 선택 (Golub-Heath-Wahba 1979).
HC3 강건 표준오차 + Huber-IRLS 5회 반복으로 유효한 추론.

> **공식 구성항목 Data 참조 — WLS 설계행렬 (5열)**
>
> | # | 변수 | 공식 | 단위 | 데이터 출처 |
> |---|------|------|------|-----------|
> | 0 | 절편 | 1 (상수) | — | — |
> | 1 | 신뢰도 | confidencePred / 100 | 무차원 (0-1) | 패턴 분석 신뢰도 |
> | 2 | 추세강도 | $\|slope_{OLS}\| / ATR$ (10봉 회귀) | 무차원 | OHLCV 종가 + ATR(14) |
> | 3 | 거래량비 | $\ln(\max(V_t / VMA_{20}, 0.1))$ | 무차원 (로그) | OHLCV 거래량 + MA(20) |
> | 4 | 변동성비 | $ATR_{14} / close$ | 무차원 (비율) | ATR(14) + OHLCV 종가 |
>
> **종속변수 (y):** h일 전진 수익률(%) − 거래비용.
> 거래비용 = $(0.03\% + 0.18\%) / h + 0.10\% / \sqrt{h}$ (KRX 수수료+세금+미끄러짐).
>
> **WLS 가중치:** $w_i = 0.995^{n-1-i}$ (최근→1.0, 최원→감쇠). 반감기 ≈ 138일.
>
> **릿지 $\lambda$:** GCV 그리드 [0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.0, 5.0, 10.0].
> Jacobi 고유분해 기반. 절편(j=0)은 정규화 미적용.
>
> **강건화:** Huber-IRLS (δ=5.8, KRX 5일 MAD 기반, 5회 반복).
> HC3 표준오차: WLS 모자행렬 $h_{ii}$ → $(1-h_{ii})^2$ 스케일링 → 샌드위치 추정.
...
Elton, Gruber, and Blake (1996). 생존편향 보정 데이터에서 로드.
...
| N0 (EB 축소) | --- | 35 | 545K KRX 패턴의 경험적 베이즈 |
| AMH λ KOSDAQ | --- | 0.00367 | 소형주 시장 빠른 알파 감쇠 |
| AMH λ KOSPI | --- | 0.00183 | 대형주 시장 느린 알파 감쇠 |
...
본 장의 시각화 설계는 다음 학문 분야의 원리를 적용한다.

| 학문 분야 | 적용 영역 | 핵심 원리 및 참조 |
|----------|----------|-----------------|
| **인지심리학** (Cognitive Psychology) | 밀도 제어, 라벨 충돌 회피 | Miller (1956) 매직 넘버 7±2 — MAX_PATTERNS=3은 인지 부하 최소화 |
| **게슈탈트 원리** (Gestalt Principles) | 9-계층 시각 위계, 색상 그룹핑 | 근접성(Proximity), 유사성(Similarity), 연속성(Continuity) |
| **정보이론** (Information Theory) | 색상 부호화, 채널 용량 | Shannon (1948) — 방향 색상(적/청) + 유형 색상(민트/보라)의 독립 채널 분리 |
| **시각 디자인** (Visual Design) | 타이포그래피, DPR 안전성 | Tufte (1983) 데이터-잉크 비율 — 9계층 중 불투명도 차등(α=0.06~1.0)으로 정보 밀도 최적화 |
| **문화인류학** (Cultural Anthropology) | 한국 시장 색상 관례 | 동아시아 적색=길조/상승, 청색=안정/하락 문화적 부호 체계 |
| **컴퓨터 그래픽스** (Computer Graphics) | Canvas2D 렌더링, DPR 처리 | 래스터화 파이프라인, 변환 초기화로 DPR 누적 방지 |

이 학문 분야들은 제3장의 수학적 출력을 **인간이 인지 가능한 시각 부호**로 변환하는 과정을 지배한다. 핵심 제약은 인지심리학에서 비롯된다: 아무리 정확한 분석이라도 사용자가 인지할 수 없으면 가치가 없다.

---

## 4.1 렌더링 엔진: TradingView Lightweight Charts v5.1.0

CheeseStock은 2,700개 이상의 종목에 대해 9개 패턴 계층과 신호·서브차트 오버레이를 동시 렌더링한다.
...
| 순서 | 계층명 | 시각 요소 | 제3장 출력 | 색상 (RGB + α) |
|------|--------|----------|-----------|----------------|
| 1 | 글로우(Glows) | 개별 캔들 수직 줄무늬 (폭 16px) | 단일 캔들스틱 패턴 | #B388FF α=0.06/0.25 |
| 2 | 브래킷(Brackets) | 2-3개 캔들 둥근 사각형 (r=4) | 이중/삼중 패턴 | #B388FF α=0.06/0.25 |
| 3 | 추세영역(TrendAreas) | 그라데이션 다각형 + 피벗 마커 | 삼각형/쐐기형 차트패턴 | #96DCC8 α=0.04 |
| 4 | 폴리라인(Polylines) | 피벗점 연결선 (W/M/넥라인) | 이중바닥/천정 | 민트 α=0.65 선폭 1.5 |
| 5 | 수평선(Hlines) | 지지/저항, 손절/목표 수평선 | S/R 클러스터링, 패턴 목표가 | 은색/#FF6B35/민트 점선 [5,3] |
| 6 | 커넥터(Connectors) | H&S 빈 원 + 어깨 연결선 | 머리어깨 피벗점 | 민트 α=0.5 점선 [2,3] |
...
| 9 | 연장선(ExtendedLines) | 화면 밖 구조선 연장 | 추세선/넥라인 | #A08830 α=0.35 점선 [8,4] |

### 4.2.1 계층 활성화 조건

각 패턴 유형은 특정 계층 조합을 트리거한다. 이 라우팅은
패턴 뷰 갱신 시점에 결정된다.
...
**계층 8 활성화 조건:** 예측 구간은 패턴에 `priceTarget`과 `stopLoss`가
모두 존재할 때만 렌더링된다. 승률 조건부 착색 적용:
승률 > 60% → 민트, 40--60% → 노랑(`#ffeb3b`), < 40% → 청색.

### 4.2.2 줌 적응형 밀도 제어

고정 `MAX_PATTERNS=3`은 기본값이나, 실제로는 가시 영역의 봉 수에 따라
동적으로 조정된다:

| 가시 봉 수 | 유효 최대 패턴 | 근거 |
|-----------|--------------|------|
| ≤ 50봉 (고배율 줌인) | 1 | 좁은 시야 → 정보 밀도 감소 필수 |
| 51--200봉 (표준 뷰) | 2 | 중간 맥락 |
| > 200봉 (축소 뷰) | 3 (기본값) | 넓은 맥락에서 다수 패턴 수용 가능 |

정렬 우선순위: (1) 활성 패턴(`priceTarget`/`stopLoss` 보유) 우선,
(2) 동순위 시 신뢰도 내림차순. 연장선도 동일한 신뢰도 정렬 후
`MAX_EXTENDED_LINES=5`로 절삭된다.

### 4.2.3 ISeriesPrimitive 재연결 시퀀스

LWC에서 `candleSeries`는 종목 변경이나 차트 유형 전환(캔들 ↔ 라인)
시마다 재생성된다. 이때 기존 프리미티브가 파괴된 시리즈에 부착된 채로
남으면 렌더링이 중단된다. 재연결 프로토콜:
...
이 프로토콜은 `patternRenderer`, `signalRenderer`, `drawingTools` 세 모듈이
동일하게 사용한다. 라인 모드에서 `_priceLine`이 `null`일 수 있으므로
반드시 null 가드가 필요하다.

### 4.2.4 라벨 충돌 회피 알고리즘

계층 7(라벨)은 겹침 방지를 위해 6회 반복 재배치를 수행한다:

1. 모든 라벨을 x좌표 순으로 정렬
2. 각 라벨의 바운딩 박스(폭: `ctx.measureText()`, 높이: 18px) 산출
3. 이전 라벨과의 겹침 검사 (x, y 모두)
4. 겹침 시 수직 오프셋 적용 (캔들 위 → 아래로, 또는 그 반대)
5. 6회 시도 후에도 겹침이면 해당 라벨 렌더링 생략
6. `MAX_PATTERNS=3` 제한이 물리적 공간 부족을 사전 방지

**결과 도트(outcome dot)**: 활성 패턴의 라벨에는 결과 상태 표시가 추가된다:
속이 빈 금색 원(활성/미확정), 빨간 점(목표 도달), 파란 점(손절 도달).
알파값은 패턴 경과 시간에 따라 감쇠하여 오래된 패턴이 자연스럽게 퇴색된다.
...
본 장의 웹 전달 아키텍처는 다음 학문 분야의 원리를 적용한다.

| 학문 분야 | 적용 영역 | 핵심 원리 및 참조 |
|----------|----------|-----------------|
| **소프트웨어 공학** (Software Engineering) | 스크립트 로드 순서, 모듈 분리 | 의존성 역전 원칙(DIP) — 19개 파일의 결정론적 로드 순서 |
| **웹 공학** (Web Engineering) | 서비스 워커, 캐싱, 오프라인 | Cache-First 전략 — 218개 공식 오프라인 가용 |
| **정보 아키텍처** (Information Architecture) | 4열 그리드, 반응형 8분기점 | Rosenfeld-Morville (2002) — A(탐색)/B(분석)/C(패턴)/D(재무) 열 분리 |
| **인간-컴퓨터 상호작용** (HCI) | 토스트, 온보딩, 가상 스크롤 | Nielsen (1994) 10대 휴리스틱 — 시스템 상태 가시성, 오류 방지 |
| **압축이론** (Compression Theory) | 복잡성 압축 (30+ 지표 → 토스트) | 정보 병목(Information Bottleneck) — 핵심 사실만 전달 |

이 학문 분야들은 제4장에서 부호화된 시각 정보를 **다양한 장치·네트워크 환경의 사용자에게 무손실로 전달**하는 과정을 지배한다. 핵심 제약은 이중 모드 동치성이다: WS 모드와 File 모드가 동일한 분석 결과를 산출해야 한다.
...
19개 JS 파일이 index.html에서 지연 로드 스크립트 태그로 직접 로드된다.
...
| A | > 0.02 | > 1.3 | ≥ 50 | 유의미한 예측력 | 녹색 |
| B | > 0.01 | > 1.1 | ≥ 20 | 최소 비무작위 신호 | 청색 |
| C | > 0.003 | — | — | 약함, 확인 필요 | 호박색 |
| D | ≤ 0.01 | ≤ 1.0 | — | 감지된 우위 없음 | 회색 |
...
|------|---------------|----------|
| 데이터 출처 | 키움증권 OCX 실시간 | 정적 JSON 파일 |
...
이론적 프레임워크(제2-3장)는 모드와 독립적이다. 이 동치성이 성립하는 조건은 두 가지이다. 첫째, 두 모드 모두 동일한 OHLCV 스키마를 입력으로 사용하므로 지표·패턴 연산의 입력 공간이 동일하다. 둘째, 신뢰도 조정 계층(제3.4절)의 거시·수급·파생 데이터는 JSON 파일에서 로드되며, 이 파일은 WS/File 모드와 무관하게 동일한 경로에서 동일한 내용을 참조한다. 따라서 데이터 신선도(staleness) 가드가 통과하는 한 분석 결과의 모드 간 편차는 발생하지 않는다.

**형식적 동치 조건:** 임의의 종목 $s$와 시점 $t$에 대해,
$\text{Analysis}(\text{OHLCV}_{s,t}, \text{MacroJSON}_t)$의 출력은
데이터 전달 경로(WS 소켓 또는 HTTP fetch)에 무의존적이다. 이는 제3장의
모든 지표(I-1..I-31), 패턴(P-1..P-32), 신호(S-1..S-22), 신뢰도 조정(CONF-계층1..6)이
OHLCV 배열과 JSON 매크로 파일만을 입력으로 취하고, 전달 메커니즘을 참조하지
않기 때문에 구성적으로 보장된다.
...
제2장: Kyle (1985) 정보거래자 모형, LSV 군집행동
...
```
제1장: VKOSPI 데이터 로드 (오프라인 수집 파이프라인)
제2장: BSM — 내재변동성이 시장 공포의 척도
       VRP — 분산위험프리미엄 = IV^2 - HV^2
제3장: 변동성 체제 분류:
