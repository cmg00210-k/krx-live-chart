...
**이론적 흐름 (8시트):** GARCH/EWMA 변동성 (2.3.1) → 극단값 이론 GEV/GPD/Hill (2.3.2) → 강건 회귀 WLS/Ridge (2.3.3) → BH-FDR 다중검정 (2.3.4) → MLE (2.3.5) → 변화점 감지 CUSUM (2.3.6) → HMM 국면 분류 (2.3.7) → 요약 (2.3.8). 조건부 변동성에서 국면 전환까지, 시장 데이터의 통계적 구조를 해부한다.
...
| RVt^((m)) = 1/MΣi₌0^(M-1) rt₋i² | 월간 성분 (M=21 KRX)  |
| rt | 일별 로그수익률  |
...
**이론적 흐름 (14시트):** 거시 — IS-LM (2.5.1) → AD-AS (2.5.2) → 테일러 준칙 (2.5.3) → 경기순환 (2.5.4) → 환율 (2.5.5) → 수익률 곡선 (2.5.6) → 재정정책 (2.5.7) → 섹터 로테이션 (2.5.8) → 경기선행지수 (2.5.9) → 고용 (2.5.10) → 무역 (2.5.11). 미시 — 수요/공급 (2.5.12) → 산업 집중도 HHI (2.5.13) → 정보비대칭 (2.5.14). 단기 균형에서 장기 구조까지, 체계적 요인을 다층적으로 포착한다.
...
**이론적 흐름 (14시트):** EMH/AMH (2.6.1) → MPT (2.6.2) → CAPM (2.6.3) → Zero-Beta (2.6.4) → ICAPM (2.6.5) → CCAPM (2.6.6) → APT (2.6.7) → Fama-French (2.6.8) → 채권수학 (2.6.9) → 신용위험 (2.6.10) → 파생상품 (2.6.11) → Black-Scholes (2.6.12) → 변동성 곡면 (2.6.13) → GZ 스프레드 (2.6.14). 효율적 시장에서 다요인 가격결정, 교차시장 신호까지 자산가격결정의 전체 계보를 추적한다.
