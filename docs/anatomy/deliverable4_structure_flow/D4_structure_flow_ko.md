...
> 본 문서는 CFA 설득 프레임워크(SCQA)에 따라 구성되었다. 설득의 정점은 Stage 3(기술력)이 아닌 **Stage 4(검증 정직성)**에 있다. OOS 백테스트에서 과적합을 거부하고 학술적 기본값을 선택한 사실이 이 문서의 핵심 메시지이다.
...
*한국의 개인 및 기관 투자자에게는 신뢰도가 검증된 기술 분석 도구가 없다. 기존 도구들은 시뮬레이션 데이터를 실제 데이터처럼 표시하고, 이론적 근거 없는 매직 넘버를 사용한다.*
...
**첫째, 신뢰도 부재.** 기존 차트 플랫폼은 패턴을 탐지하면 단순히 "강세" 또는 "약세" 라벨을 붙인다. 그 패턴이 현재 거시경제 환경, 신용 리스크, 유동성 조건에서 얼마나 신뢰할 수 있는지에 대한 정보는 제공하지 않는다. 투자자는 패턴의 기하학적 품질과 시장 맥락에서의 예측력을 구분할 방법이 없다.

**둘째, 데이터 투명성 결여.** 많은 플랫폼이 재무 데이터의 출처를 밝히지 않으며, 일부는 시뮬레이션으로 생성된 데이터를 실제 데이터인 것처럼 표시한다. 투자자가 보는 PER, PBR, ROE 수치가 DART(금융감독원 전자공시시스템)에서 온 것인지, 아니면 임의로 생성된 것인지 알 수 없다.

**셋째, 학술적 근거 없는 매직 넘버.** "RSI가 70 이상이면 과매수"와 같은 고정된 임계값은 Wilder(1978)의 원래 연구에서 미국 선물 시장 14일 기준으로 제시된 것이다. 한국 주식 시장의 변동성 구조, +-30% 가격 제한, 한국 특유의 투자자 구성(외국인/기관/개인 3분법)을 반영한 적응형 기준값은 존재하지 않았다.

CheeseStock은 이 세 가지 문제에 대한 공학적 해답이다.

---

## 1.2 플랫폼 정체성

CheeseStock(cheesestock.co.kr)은 한국거래소(KRX) 상장 전 종목(KOSPI + KOSDAQ, 2,696개)을 대상으로 하는 브라우저 네이티브 기술 분석 플랫폼이다. 실시간 차트, 45개 유형의 자동 패턴 탐지(Pattern Detection), 10-함수 거시-미시 신뢰도 보정 체인, K-IFRS 재무 패널을 단일 페이지 웹 애플리케이션으로 제공하며, 설치가 필요 없고 서버 측 비즈니스 로직이 전혀 없다.

아키텍처를 정의하고 모든 후속 결정을 제약하는 세 가지 설계 선택:

1. **빌드 시스템 없음(No Build System)**: 모든 JavaScript가 브라우저에서 직접 실행된다. webpack, rollup, esbuild, 트랜스파일러가 없다. 전역 변수(Global Variable)와 로드 순서(Load Order)가 모듈 임포트를 대체한다.
...
스크립트 로드 순서는 19개 메인 스레드 파일 전체에서 고정되어 있다(`CLAUDE.md` 아키텍처 섹션 참조). 순서 변경 시 참조 오류(Reference Error)가 발생하는데, 앞선 스크립트가 내보낸 전역 변수를 이후 스크립트가 소비하기 때문이다.
...
| 백테스트 인스턴스 | 303,956 | 2,704 종목 x 패턴 유형, 2023-2025 |
...
동일한 애플리케이션 코드가 `api.js initFromIndex()`의 도메인 감지를 통해 양쪽 컨텍스트를 처리한다. 파일 모드가 먼저 렌더링되고, WebSocket은 사용자를 차단하지 않고 백그라운드에서 업그레이드된다.
...
핵심 아키텍처 불변식: **분석 파이프라인은 모드를 인지하지 않는다.** `patternEngine.analyze(candles)`는 캔들이 JSON 파일에서 왔든 라이브 WebSocket 틱 스트림에서 왔든 동일한 데이터 구조를 수신한다. 모드 인식은 전적으로 `api.js`와 `realtimeProvider.js`에 캡슐화되어 있다.
...
기술 분석은 그 근거가 되는 데이터만큼만 신뢰할 수 있다. 한국 시장 맥락에서 세 가지 구조적 위험이 데이터 출처 관리를 비자명(non-trivial)하게 만든다:

1. **소스 이질성**: 가격 데이터(KRX/pykrx), 거시 데이터(ECOS/FRED/KOSIS), 재무 데이터(DART), 파생상품 데이터(KRX API)는 각각 다른 업데이트 주기, 인증 체계, 실패 모드를 따른다.

2. **무성 열화(Silent Degradation)**: `daily_update.bat` 파이프라인은 전진 실패(fail-forward) 모델을 사용한다 -- 초기 건강 확인 이후의 모든 다운로드 단계는 WARNING을 발생시키고 계속 진행한다. 거시 다운로드의 완전한 실패는 사용자에게 보이는 오류를 생성하지 않는다; 오직 신뢰도 체인이 해당 조정 인자를 조용히 건너뛸 뿐이다.

3. **시드 데이터 오염**: DART 재무 데이터가 없는 종목에 대해 시스템은 코드 해시에서 결정론적 의사 난수 재무 데이터를 생성한다(시드 데이터). 시드 데이터를 실제 데이터로 표시하는 것은 사용자를 중대하게 오도하는 것이다. 신뢰 체계가 렌더 계층에서 이를 방지한다.
...
| 스크립트 | 입력 | 출력 | 핵심 방법론 |
|---------|------|------|-----------|
| compute_macro_composite.py | macro_latest, bonds_latest, kosis_latest | macro_composite.json | MCS v2 (8-요인, 0-100), Taylor Rule |
| compute_options_analytics.py | options_latest, bonds_latest, kospi200_daily | options_analytics.json | BSM IV (Newton-Raphson), 스트래들 내재 변동 |
| compute_capm_beta.py | OHLCV, 시장 지수, bonds_latest, financials | capm_beta.json | OLS 베타, Scholes-Williams, Merton DD |
| compute_basis.py | derivatives_summary, bonds_latest | basis_analysis.json | 보유 비용(Cost-of-carry), 초과 베이시스 z-score |
| compute_eva.py | financials, capm_beta, macro_latest | eva_scores.json | NOPAT, WACC, EVA 스프레드, EVA 모멘텀 |
| compute_hmm_regimes.py | OHLCV(시가총액 가중 프록시) | hmm_regimes.json | Baum-Welch EM, 2-상태 가우시안 HMM |
| compute_flow_signals.py | 종목별 투자자, hmm_regimes | flow_signals.json | 외국인 모멘텀(20일 MA), 개인 역방향 |
| compute_bond_metrics.py | bonds_latest | bond_metrics.json | Macaulay/Modified Duration, DV01, 볼록성(Convexity) |
| compute_illiq_spread.py | 종목별 OHLCV | illiq_spread.json | Amihud ILLIQ, Roll(1984) 스프레드 |
| compute_survivorship_correction.py | pattern_performance(상장 + 상장폐지) | survivorship_correction.json | 상장/통합 간 delta_wr |
...
**계층 2 -- 연산 스크립트 가드**: `compute_eva.py` 및 기타 연산 스크립트는 `source in ('seed', 'demo')`인 재무 입력을 거부한다. 이는 시드 데이터 EVA 점수가 `eva_scores.json`에 나타나는 것을 방지한다.

**계층 3 -- 동종 그룹 필터**: `sidebarManager`의 동종 그룹(Peer Group) 계산은 시드로 태그된 종목을 업종 비교 풀에서 제외한다. 시드 종목이 업종 중위값 PER/PBR을 부풀리거나 축소할 수 없다.

이 보장의 유일한 실패 벡터는 `data/financials/{code}.json` 파일이 조작된 데이터를 포함하면서 `source: 'dart'`를 잘못 표시하는 경우다. 이는 정상 파이프라인(`download_financials.py`는 DART API 응답 상태로부터만 `source`를 설정)을 통해서는 발생할 수 없으며, 수동 파일 변조가 필요하다.
...
| 파일 | 생산자 | JS 소비자 | 필수 키 |
|------|-------|----------|---------|
| `data/index.json` | download_ohlcv.py / update_index_prices.py | `ALL_STOCKS` (api.js) | `stocks` 배열 |
| `data/{market}/{code}.json` | download_ohlcv.py | `dataService.getCandles()` | OHLCV 캔들 배열 |
| `data/{code}_{tf}.json` | generate_intraday.py | `dataService.getCandles()` | 장중 OHLCV |
| `data/market/kospi_daily.json` | download_market_index.py | `backtester._marketIndex` | `time`, `close` 배열 |
| `data/market/kosdaq_daily.json` | download_market_index.py | `financials.js` 동종 그룹 컨텍스트 | `time`, `close` 배열 |

### 범주 2: 거시 경제와 통화 정책

| 파일 | 생산자 | JS 소비자 변수 | 필수 키 |
|------|-------|---------------|---------|
...
| `data/derivatives/basis_analysis.json` | compute_basis.py | `_derivativesData`에 병합 | 없음 |
| `data/vkospi.json` | download_vkospi.py | `_macroLatest.vkospi`에 주입 | 없음 |

### 범주 4: 행동 분석과 백테스트

| 파일 | 생산자 | JS 소비자 | 비고 |
|------|-------|----------|------|
| `data/backtest/capm_beta.json` | compute_capm_beta.py | `backtester._capmBeta`, `financials.js` | CAPM 베타 + Merton DD |
| `data/backtest/eva_scores.json` | compute_eva.py | `_evaCache` (financials.js) | 종목별 EVA 스프레드 |
| `data/backtest/hmm_regimes.json` | compute_hmm_regimes.py | `backtester._behavioralData` | 2-상태 HMM; 30일 적시성 게이트 |
| `data/backtest/illiq_spread.json` | compute_illiq_spread.py | `backtester._behavioralData` | Amihud ILLIQ, Roll 스프레드 |
| `data/backtest/flow_signals.json` | compute_flow_signals.py | `_flowSignals` | HMM 라벨 + 외국인 모멘텀 |
...
| `data/backtest/rl_policy.json` | compute_rl_policy.py(오프라인) | `backtester._rlPolicy` | IC < 0 게이트 기각 |

### 범주 5: 기본적 재무 데이터

| 파일 패턴 | 생산자 | JS 소비자 | 신뢰 티어 |
|----------|-------|----------|----------|
| `data/financials/{code}.json` (2,607개) | download_financials.py (DART) | `_financialCache` via `getFinancialData()` | Tier 1(dart) / Tier 3(seed 필터링) |
| `data/macro/ff3_factors.json` | compute_ff3_factors.py | `_ff3Cache` (financials.js) | Fama-French 3-요인 |
| `data/sector_fundamentals.json` | download_sector.py | `_sectorData` (app.js) | 동종 그룹 PER/PBR/PSR 중위값 |
...
| 파일 | 생산자 | 상태 |
|------|-------|------|
| `data/derivatives/hedge_analytics.json` | compute_hedge_ratio.py | JS 소비자 없음; 디스크에 파일 부재 |
| `data/backtest/krx_anomalies.json` | compute_krx_anomalies.py | JS 소비자 없음; 배포에서 제외(.cfignore) |
...
파이프라인의 전체 아키텍처는 **의도적 전진 실패(fail-forward by design)**이다: Step 0(API 건강 확인)만이 유일한 하드 중단(hard-abort) 게이트다. Step 1-18은 모두 `WARNING`을 발생시키고 계속 진행하므로, 개별 단계 실패에 관계없이 파이프라인은 항상 종료 코드 0으로 종료한다. 이는 의도적 복원력 선택이다 -- 부분 데이터가 데이터 없음보다 낫다 -- 그러나 무성 열화 위험을 생성한다.
...
`compute_options_analytics.py`에 유효한 입력이 없을 때(Step 12 실패) `{ status: 'no_data', analytics: null }`을 기록한다. `appWorker.js`가 `_optionsAnalytics.analytics.straddleImpliedMove`를 읽을 때 null 체인 검사가 없으면 런타임에 `TypeError`를 발생시킨다. V7 감사에서 null 가드가 존재하지만 최상위 null만 커버하고 중첩 필드 접근은 완전히 가드되지 않음을 확인했다.
...
| 파일 | 가드 필드 | 가드 값 | 실패 동작 |
|------|----------|---------|----------|
| `investor_summary.json` | `source` | `'sample'` | `_investorData = null`; CONF-5 외국인 수급 건너뜀 |
| `shortselling_summary.json` | `source` | `'sample'`, `'unavailable'` | `_shortSellingData = null`; CONF-5 공매도 비율 건너뜀 |
| `derivatives_summary.json` | `source` | `'sample'`, `'demo'` | `_derivativesData = null`; CONF-5 베이시스/PCR 건너뜀 |
| `flow_signals.json` | `status` | `'error'` | `_flowSignals = null`; CONF-7 HMM/수급 보너스 건너뜀 |
| `options_analytics.json` | `status` | `'error'` | `_optionsAnalytics = null`; CONF-7 내재 변동 건너뜀 |
| `macro_composite.json` | `source` | `'sample'`, `'demo'` | `_macroComposite.mcsV2 = null`; MCS 조정 건너뜀 |
| `market_context.json` | `source` | `'demo'` | `_marketContext` 컨텍스트 리셋; CCSI 조정 건너뜀 |
...
| 로더 | 파일 | 트리거 | 소비자 |
|------|------|--------|-------|
| `getFinancialData(code)` | `data/financials/{code}.json` | 종목 선택 | `financials.js` 컬럼 D |
| `_loadBehavioralData()` | `illiq_spread`, `hmm_regimes`, `csad_herding`, `disposition_proxy`, `survivorship_correction` | `backtester.init()` | 신뢰도 체인 CONF-8 |
| `_loadCAPMBeta()` | `data/backtest/capm_beta.json` | `backtester.init()` | CAPM 베타 + Merton DD 표시 |
| `_loadMarketIndex()` | `data/market/kospi_daily.json` | `backtester.init()` | 베타 회귀 벤치마크 |
...
*시스템의 지적 재산(IP)은 패턴 탐지 자체가 아니라, 탐지된 패턴에 거시경제 레짐, 신용 리스크, 유동성 조건, 생존자 편향을 반영한 10-함수 승법적 신뢰도 보정 체인에 있다.*
...
**왜 3계층인가?** 이 분리는 고전적 신호 처리를 따른다: 측정(지표), 특징 추출(패턴), 의사결정(신호). 지표는 시장 불가지론적(market-agnostic) 순수 함수이다; 패턴은 도메인 특정 인식을 요구한다; 신호는 다중 소스 합류(confluence)를 요구한다. 백테스터(`backtester.js`)는 검증 계층으로 병행 실행된다 -- 그 출력은 UI와 오프라인 교정에 공급되며 탐지 파이프라인에는 공급되지 않는다.
...
캐시는 이름과 매개변수로 키된 지연 접근자(lazy accessor)를 사용한다(예: `"ma_20"`, `"bbEVT_20_2"`). 첫 접근 시 계산하여 저장; 이후 접근은 즉시 반환한다. **핵심 제약**: 캐시가 지연 평가를 위한 함수 참조를 저장하므로, `postMessage()`를 통해 Worker 경계를 넘을 수 없다(구조화 복제가 함수에 실패). Worker는 자체 캐시를 구축한다.
...
| CONF | 함수명 | 데이터 소스 | 학술적 근거 | 조정 범위 | 방향 | 등급 혼합 |
|------|--------|-----------|-----------|----------|------|---------|
| 1 | Market Context | market_context.json | Lemmon & Portniaguina(2006), Richards(2005) | [0.55, 1.35] | 매수 중심 | 4[D] |
| 2 | RORO Regime | 5-요인 복합(거시, 채권, 투자자) | Baele, Bekaert & Inghelbrecht(2010) | [0.92, 1.08] | 방향성 | 6[C], 6[D] |
| 3 | Macro 11-Factor | macro_latest, bonds_latest, kosis_latest, macro_composite | IS-LM, Taylor(1993), Stovall(1996), Gilchrist & Zakrajsek(2012) | [0.70, 1.25] | 양방향 | 24[B-C], 21[D] |
| 4 | Micro(ILLIQ/HHI) | 캔들(OHLCV), ALL_STOCKS | Amihud(2002), Kyle(1985) | [0.80, 1.15] | 양방향 | 5[B-C] |
| 5 | Derivatives 6-Factor | derivatives, investor, etf, shortselling, basis | Bessembinder & Seguin(1993), Pan & Poteshman(2006) | [0.70, 1.30] | 방향성 | 6[B-C], 8[D] |
| 6 | Merton DD | 캔들, financials, 시가총액, bonds | Merton(1974), Bharath & Shumway(2008) | [0.75, 1.15] | 방향성 | 3[A-B], 5[D] |
| 7 | Phase 8(MCS/HMM/IV) | macro_composite, flow_signals, options_analytics | MCS Doc30, HMM Doc46, Simon & Wiggins(2001) | 최종 [10,100] | 양방향 | 4[B-C], 4[D] |
| 8 | Survivorship | backtester._survivorshipCorr | Elton, Gruber & Blake(1996) | [0.92, 1.00] | 매수만 | 3[C-D] |
| 9 | Signal Macro | macro_latest, bonds_latest(신호 경로) | CONF-3과 동일 | [0.70, 1.25] | 복합 특정 | 상속 |
| 10 | Wc Injection | patterns 평균 wc | 메타데이터 전용 | 변경 없음 | 양방향 | N/A |
...
**CONF-2 RORO Regime**는 5-요인 가중 복합에서 3-상태 레짐(리스크 온 / 중립 / 리스크 오프)을 분류한다: VKOSPI/VIX(0.30), AA- 신용 스프레드(0.10), US HY 스프레드(0.10), USD/KRW(0.20), MCS(0.15), 투자자 정렬(0.15). 히스테리시스(진입 +/-0.25, 퇴출 +/-0.10)가 레짐 진동을 방지한다. 클램프 [0.92, 1.08]은 의도적으로 좁다 -- Section 3.14의 상호작용 효과 참조.
...
| 순위 | 함수 | 최대 영향 | 트리거 빈도 |
|------|------|---------|-----------|
| 1 | CONF-6 Merton DD | -25% 매수 | 드묾(신용 손상 종목만) |
...
> **[핵심 요약]** 303,956건 패턴 백테스트, 7-게이트 검증(다중검정 BH-FDR, Hansen SPA, WFE ≥ 0.3 게이팅). IC = 0.051(t = 3.73, p < 0.001) — 업계 기준 하위-중간이나 단기 TA에는 통계적 유의. **두 차례 OOS 교정 모두 기각 → 학술적 기본값 채택. 이것이 과적합 회피의 증거.**
...
**색상 체계**: 한국 관례(상승=빨강 `#E05050`, 하락=파랑 `#5086DC`). 패턴 오버레이는 방향 색상을 사용하지 않음(민트/보라/은색/금색). 대시 패턴으로 확인 상태 인코딩(실선=확인, `[5,3]`=미확인).
...
**알려진 한계.** IC 계산 경로의 회귀 계수는 전체 훈련 세트에서 적합되며, OOS 폴드별로 재적합되지 않는다. 진정한 재귀적 OOS IC(WFE 경로, Gate 2에서 가용)는 더 낮을 것이다. 백테스터에서 Warning W-4로 표시됨.
...
**1. 진정한 실시간 OOS 검증 없음.** 모든 검증은 과거 OHLCV 데이터에서 실행. 배포 조건에서 신뢰도 점수가 순방향 수익률을 예측하는지 검증하는 페이퍼 트레이딩이나 라이브 체결 루프가 존재하지 않음.
...
**5. 종목 간 MTC 부재.** BH-FDR은 종목별로만 적용. 2,700 x 225 = 607,500 유효 검정 건수에 포트폴리오 수준 보정 없음.
...
*알려진 한계에 구체적 개선 경로가 있다. 미래 방향은 이론적 기반 위에서의 점진적 확장이다.*
...
- **hw(Historical Weight)**: 백테스트 기반 패턴 유형별 역사적 가중치
- **vw(Volatility Weight)**: ATR 기반 변동성 조건 반영
- **mw(Market Weight)**: 거시 레짐 상태 반영

Wc는 현재 CONF-10에서 메타데이터로 주입되며 신뢰도를 직접 수정하지 않는다. 향후 Stage에서 Wc가 렌더링 우선순위(예측 구간 투명도, 다이아몬드 크기)와 복합 신호 가중에 직접 반영될 예정이다.
...
**MCS** -- 거시 복합 점수(Macro Composite Score): ECOS, FRED, KOSIS 입력에서 `compute_macro_composite.py`가 계산하는 8-요인 복합 점수(0-100); v1(단순)은 CONF-3에서, v2(전체)는 CONF-7에서 사용.
