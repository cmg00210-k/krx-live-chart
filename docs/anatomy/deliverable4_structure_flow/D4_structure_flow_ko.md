...
이 보장의 유일한 실패 벡터는 `financials/{code}.json` 파일이 조작된 데이터를 포함하면서 `source: 'dart'`를 잘못 표시하는 경우다. 이는 정상 파이프라인(`download_financials.py`는 DART API 응답 상태로부터만 `source`를 설정)을 통해서는 발생할 수 없으며, 수동 파일 변조가 필요하다.
